<template>
  <article class="mainContainer">
    <fence
      ...
      @mainShow="getbimPersonList()"></fence>
    <!--    底部 信息栏 显示状态-->
    <div
      v-show="!mainShow"
        ...
        <div class="leftClass">
          ...
          <div class="tableContainer">
            ...
            >
              ...
              />
              <template slot="personName" slot-scope="text, record, index, column">
...
export default class personnelEquipment extends Vue {
  ...
  mounted() {
    this.getbimPersonList();
    //定时刷新界面
    this.timer = setInterval(() => {
      //有轨迹或处于围栏查看界面  不刷新
      if (this.traceId.length === 0 && !this.mainShow) {
        console.log('人员刷新');
        this.updateBimPersonLocation();
        // this.getbimPersonList();
      }
    }, 60000);
  }
  ...
  drawPie(color, name, data) {
    // @ts-ignore
    const myChart = echarts.init(document.getElementById('personEchart'));
    myChart.clear()
    myChart.setOption({
      color: color,
      legend: {
  ...
  getbimPersonList() {
    ...
    Api.getbimPersonList({
      appCode: this.projectCode,
      projectName: this.projectName
    }).then((res) => {
      if (res.errcode !== 0) return
      if (!res.data) return
      this.personTableData = res.data.dataList;
  ...
  updateBimPersonLocation() {
    let dataList:Array<any>=[]
    this.personTableData.forEach(function (v) {
      var tempt={
        id:v.id,
    ...
    Api.updateBimPersonLocation({appCode: this.projectCode, dataList: dataList}).then(res => {
      if(res.errcode===0) this.getbimPersonList()
    });
  }
