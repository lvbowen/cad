...
export default class shipEquipment extends Vue {
  ...
  drawPie(color, name, data) {
    // @ts-ignore
    const myChart = echarts.init(document.getElementById('carEchart'));
    myChart.clear()
    myChart.setOption({
      color: color,
      legend: {
  ...
  getbimEquipList() {
    ...
    Api.getbimEquipList({
      ...
      if (!res.data) return
      this.carTableData = res.data.dataList;
  ...
  updateBimEquipLocation(){
    ...
    this.carTableData.forEach(function (v) {
      var tempt={
        id:v.id,
