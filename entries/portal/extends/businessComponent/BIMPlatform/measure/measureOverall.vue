      ...
      <main class="bottomMain">
        <div style="width:75%" v-show="!isDetailShow">
          ...
          >
            ...
            >
              <div
                :class="record.payState === '进行中'?'doing':record.payState === '已完成'?'done':'todo'">
                {{
                  record.payState
                }}
              </div>
            ...
            >
              <p
                :class="record.measureState === '进行中'?'doing':record.measureState === '已完成'?'done':'todo'">
        ...
        <div v-show="isDetailShow" style="width:100%;">
          ...
          <img
            :src="closeIcon"
            style="float: right;font-size:25px;margin-top: 1%"
            @click="() => {this.isDetailShow = false}"/>
        </div>
      </main>
...
import {
  Card, Button, Icon, Table, Input, Modal, Divider, Col, Row, Tree, Dropdown, Menu, Spin, Select
} from '@h3/antd-vue';
...
import closeIcon from '../../../../src/assets/extends/bim/close.png'
...
export default class MeasureOverall extends Vue {
  ...
  initDetail(id) {
    axios
      .get(this.bimURL + `api/measurePayment/cbsMeasure/getByCBSId`, {
        params: {
          projectCode: this.projectCode,
          cbsId: id,
          periodId: this.periodID === "all" ? "" : this.periodID
        }
      })
      .then(res => {
  ...
  initPeriod() {
    ...
    axios
      .get(
        this.bimURL + "bim/measure/getAllPeriod", {
          params: {
      ...
      .then(res => {
        //@ts-ignore
        if (res.errcode === 0) {
          this.periodData.push({id: "all", measurePeriod: "全部"});
          for (let i = 0; i < res.data.length; i++) {
            this.periodData.push(res.data[i])
          }
        } else {
          //@ts-ignore
  ...
  initTree(item) {
    ...
    this.initPeriod()
    //配置接口路径
    let url = this.bimURL + "bim/measure/getCBS";
  ...
  mounted() {
    ...
    this.tableData = [];
    this.periodID = "";
    this.periodName = "周期选择";
    //设置默认
    this.buttonIndexLeft = "计量支付";
    this.initTree("计量支付");
  ...
  async onCheck(checkedKeys, info) {
    ...
    if (info == "start") {
      ...
      this.$emit('getShowHideData', true, '计量支付', this.checkedKeys);
    } else {
      if (info.checked === false) {
        this.checkedKeys.push(info.node.eventKey);
        this.$emit('getShowHideData', false, '计量支付', this.checkedKeys);
      } else {
        this.checkedKeys.push(info.node.eventKey);
        this.$emit('getShowHideData', true, '计量支付', this.checkedKeys);
      }
  ...
  onLoadData(treeNode) {
    ...
    return new Promise(resolve => {
      ...
      let params = {
        projectCode: this.projectCode,
        projectName: this.projectName,
        parentId: treeNode.$vnode.data.key,
      };
      axios
        .get(url, {
          params: params
  ...
  onSearch(value) {
    ...
    if (value.length == 0) {
      ...
      axios
        .get(
          this.bimURL + 'bim/measure/getCBSByName', {
            params: {
              "projectCode": this.projectCode,
              "projectName": this.projectName,
              "name": value
  ...
  async onSelect(selectedKeys) {
    if (selectedKeys.length == 0) {
      ...
      this.tableData = [];
      this.isDetailShow = false;
      if (this.buttonIndexLeft != "otherButtons") {
        this.tableLabel = [];
      }
      //清空表格选择
      this.selectedRowKeys = [];
      let url = this.bimURL + "bim/measure/getAllMBS";
      // 构造输入
      let params = {
        projectCode: this.projectCode,
        detailId: this.selectedKeys[0],
      ...
      axios
        ...
        .then(res => {
          ...
          if (res.errcode == 0) {
            ...
            let resData = res.data;
            for (let i = 0; i < resData.cbsMbsTotalList?.length; i++) {
              this.$set(resData.cbsMbsTotalList[i], "key", resData.cbsMbsTotalList[i].id);
            }
            this.tableData = resData.cbsMbsTotalList;
            this.constTable = resData.cbsMbsTotalList;
            for (let key in resData.measureStatics) {
              let tempt = resData.measureStatics;
              this.measureStatic.push({name: key, value: tempt[key]});
              this.measureTotal += Number(tempt[key])
            }
            for (let key in resData.payStatics) {
              let tempt = resData.payStatics;
