          ...
          <a-col :span="7">
            ...
            <div class="leftDiv">
              ...
              <a-dropdown :trigger="['click']" class="leftDropdown">
                <a-menu slot="overlay">
                  <template v-for="menuItem in schemaMenu">
                    <a-menu-item
                      :key="menuItem.id"
                ...
                <a-button size="small">{{ schema ? schema.schemeName : '' }}
                  <a-icon type="down"/>
                </a-button>
              </a-dropdown>
              <span>期号</span>
              <a-dropdown :trigger="['click']" class="leftDropdown">
                <a-menu slot="overlay">
                  <template v-for="menuItem in periodMenu">
                    ...
                    >
                      {{ menuItem.measurePeriod }}
                    </a-menu-item>
                  </template>
                </a-menu>
                <a-button size="small">{{ period.measurePeriod }}
                  <a-icon type="down"/>
                </a-button>
              </a-dropdown>
              <a-table
...
import {Carousel, Divider, Row, Col, Dropdown, Button, Menu, Table, Icon} from '@h3/antd-vue';
...
export default class projectView extends Vue {
  ...
  //合同信息管理
  schemaMenu: Array<any> = [];
  periodMenu: Array<any> = [];
  schema: { id: string, schemeName: string } | null = null;
  period: { id: string, measurePeriod: string } | null = null;
  tableTree: Array<any> = [];
  ...
  drawPie(record) {
    ...
    const total: number = record.finalTotalPrice;
    pieData.push({
      name: '上期末计量',
  ...
  getCbsMeasure(record) {
    console.log('#019ce3 !important', record)
    Api.getByCBSAndPeriodPageV3({
      projectCode: this.projectCode,
      periodId: record.tg04cMeasureperiodFk as string,
      cbsCode: record.ccbs as string,
      schemaId: record.tg04aContractschemeFk as string,
    })
      .then(res => {
        if (res.errcode !== 0) return
  ...
  getPeriodByContract() {
    Api.getAllPeriodBySchemaV3({
      appCode: this.projectCode,
      schemaId: this.schema?.id as string
    })
      .then(res => {
        if (res.errcode !== 0) return;
  ...
  getCbsTree() {
    ...
    Api.getMeasureDetailChildsV3({
      projectCode: this.projectCode,
      schemaId: this.schema?.id as string,
      periodId: this.period?.id as string,
      projectName: this.projectName
    })
      .then(res => {
          console.log('ssssssssss0', res.data)
          if (res.errcode !== 0) return;
    ...
    return new Promise((resolve) => {
      ...
      Api.getMeasureDetailChildsV3({
        ...
        .then((res) => {
          ...
          this.editListTree(_this.tableTree, c.id, c);
        })
      resolve(c)
    })
  }
