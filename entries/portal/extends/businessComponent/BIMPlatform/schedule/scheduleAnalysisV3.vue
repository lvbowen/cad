    ...
    <div class="progressAnalysisGannt">
      <div class="checkGannt">
        ...
        >
          ...
          <CheckBox value="1">里程碑V3</CheckBox>
...
import { getscheduleAnalyseStateV3, getPageLeafList, getModel, getStateByReportDateV3 } from "../server/index.js";
...
export default class scheduleAnalysisV3 extends Vue {
  id: string = "";
  planId: string = "";
  input: string = "";
  startime: string = "";
  ...
  modelTimeColor() {
    ...
    if ( this.AllmodelTimeShow ) {
      ...
      getStateByReportDateV3(this.projectCode, this.$route.query.projectName, this.nodeId, this.startime, this.endtime).then(res=>{
  ...
  ProgresStatus() {
    ...
    getscheduleAnalyseStateV3(
      ...
      this.scheduleData = res.data.progressStateDTOList;
      //@ts-ignore 进度状态
      this.investmentData1.series = res.data.progressStateDTOList?.map(
        ( item ) => {
  ...
  messagEchart( val ) {
    ...
    if ( val.name == "完成产值" ) {
      ...
      this.scheduleData.forEach( ( e ) => {
        ...
        if ( e.model ) {
          //@ts-ignore
          if ( e.progressStateDesc == val.name && val.value == e.money ) {
            //@ts-ignore
