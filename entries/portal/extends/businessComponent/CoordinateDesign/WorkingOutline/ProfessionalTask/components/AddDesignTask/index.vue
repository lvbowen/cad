<template>
  <a-modal
    ...
    @cancel="addDesignTaskVisible = false"
    :footer="null">
    <template #title>
      <div class="clearfix">
        <div class="left">
        ...
        <div class="right designButtons">
          <a-button @click="addDesignTask" class="designButton" type="primary">确认</a-button>
          <a-button @click="addDesignTaskVisible = false" class="designButton">取消</a-button>
        </div>
      </div>
    </template>
    <div class="multipleTitle">
      <div class="paddingRight10px">当前选中项:</div>
      <div style="flex:1" class="flex flex-center-align">
        <span class="paddingRight10px"><em style="color: #2970ff">{{ selectItemNames.length }}</em>/{{ totalCount }}</span>
        <a-tooltip title="选项显示规则见后台管理表单配置！">
          <a-icon type="question-circle" />
        </a-tooltip>
        <!-- <span class="link">显示名称</span> -->
        <!-- <p :title="selectItemNames.join(';')" style="color:#82d559">{{ selectItemNames.join(';') }}</p> -->
      </div>
    </div>
    <a-form-model
      ref="productionForm"
