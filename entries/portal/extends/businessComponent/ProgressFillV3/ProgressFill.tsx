...
import { getReportMBSInfo, getReportMBSList, getAddMBSList } from "./index.js"
...
export default class ProgressFill extends Vue {
  ...
  private dialogTable: boolean = false;
  ...
  private Quantities: number = 0
  private isAllExpandShow: boolean = false

  private gridData: Array<any> = []
  private recordData: Array<any> = []
  private options: Array<any> = []
  private checkTableLists: Array<any> = []
  private selectValue: string = ""
  private dialogRatioValue: boolean = false
  private dialogTableRecord: boolean = false
  private ratioValue: number = 0
  private reportBsId: string = ""
  private cumulativeReportRatio: number = 0
  private submitText: string = "提交"
  private disabled: boolean= false
  private scheduleReportMbsCmdList: Array<any>= []
  ...
  async mounted() {
    ...
    document.addEventListener("click", e => {
      ...
      const calendar = document.getElementsByClassName("ant-calendar-picker-container");
      const antmodal = document.getElementsByClassName("ant-modal");
      //@ts-ignore
      if (!iconClose?.contains(e.target) && this.submitData.show && calendar.length !== 1 && antmodal.length !== 1) {
        this.closeSubmitPanel()
      }
    })
  }
    ...
    return (
      <article class={Class.main} ref={'MainContainer'}>
        ...
        <main class={Class.mainContainer}>
          <section class={Class.container}>
            ...
            <article>
              <nav class={Class.optNav}>
                ...
                />
                <Button onClick={this.isAllExpand}>全部展开</Button>
                <Tooltip placement={'right'} overlayClassName={Class.filterContainers}
          ...
          <section id="iconClose" class={`${Class.container} ${submitData.show ? Class.show : Class.hide}`}>
            <nav>
              {
            ...
            </nav>
            {
              ...
              <article class={Class.planDetailContainer}>
                ...
                <nav>
                  <Icon src={'title'} />
                  <span>构件信息</span>
                  <span onClick={() => this.ToDetailed(questList.selectRow,"构件信息")} style={"margin-left: auto;color:rgb(64, 169, 255);cursor: pointer; font-size: 13px"}>点击设置</span>
                </nav>
                <aside>
                  {
                    <span class={Class.singleValue}>已关联构件数量：{questList.selectRow?.mbsNum}</span>
                  }
                </aside>
                ...
                <div class={Class.planDetailRow}>
                  <span>工程量</span>
                  <Input
                    v-model={this.Quantities}
                    onBlur={this.onQuantitiesBlur}
                    placeholder={"请输入工程量"}
                  />
                </div>
                <div class={Class.planDetailRow}>
                  <span>{locale?.Progress.NowFillProduction}</span>
        ...
        <Modal
          ...
          footer={false}
          width="1000px"
          onCancel={(e) => { this.dialogTable = false }}
          visible={this.dialogTable}>
          <div style="display: flex; justify-content: space-between;">
            <div>
            <span style="margin-right: 10px">关键部位填报记录：</span>
              <elSelect disabled={this.disabled} onChange={this.selectChange} v-model={this.selectValue} placeholder="请选择">
              {this.options?.map((item=>{
                 return (
                    <elOption
                      key={item.id}
                      label={item.reportDate + "-" + item.userName + "-" + item.reportRatio + "%"}
                      value={item.id}>
                    </elOption>
                 )
              }))}
            ...
            <div>
              <Button onClick={this.BatchFilling} type={'primary'}>批量填报</Button>
            </div>
          </div>
          <elTable onSelection-change={this.handleSelectionChange} data={this.gridData} height="600">
          <elTableColumn
              type="selection"
              width="55">
            </elTableColumn>
            <elTableColumn
              ...
              width="150"
            ></elTableColumn>
            <elTableColumn align="center" property="mbsName" label="构件名称"></elTableColumn>
            <elTableColumn align="center" property="ratio" scopedSlots={{ default: this.columnRender}} label="构件完成比例">
            </elTableColumn>
            <elTableColumn align="center" label="填报记录" scopedSlots={{ default: this.toDetails}}></elTableColumn>
          </elTable>
          <div style="text-align: center;margin-top: 15px;">
            <Button onClick={() => { this.dialogTable = false }} style="margin-right: 30px">取消</Button>
            <Button onClick={this.submitRecord} type={'primary'}>{this.submitText}</Button>
          </div>
        </Modal>
        <Modal
          title="填报记录"
          ...
          footer={false}
          width="1000px"
          onCancel={(e) => { this.dialogTableRecord = false }}
          visible={this.dialogTableRecord}>
          <elTable data={this.recordData} height="640">
            <elTableColumn
              type="index"
              label="序号"
              width="100"
              align="center"
            ></elTableColumn>
            <elTableColumn align="center" property="reportDate" label="填报日期"></elTableColumn>
            <elTableColumn align="center" property="mbsName" label="构件名称"></elTableColumn>
            <elTableColumn align="center" property="ratio" label="构件完成比例"></elTableColumn>
            <elTableColumn align="center" property="userName" label="填报人"></elTableColumn>
          </elTable>
        </Modal>
        <Modal
          title="批量填报"
          ...
          footer={false}
          width="300px"
          onCancel={(e) => { this.dialogRatioValue = false }}
          visible={this.dialogRatioValue}>
            <ASlider v-model={this.ratioValue} />
            <Button onClick={() => { this.dialogRatioValue = false }} style="margin-right: 30px">取消</Button>
            <Button onClick={this.ratioOk} type={'primary'}>确定</Button>
        </Modal>
      </article>
    );
  }
  ...
  private handleSelectionChange(val) {
    this.checkTableLists = val
  }

  private BatchFilling() {
    this.ratioValue= 0
    if(this.checkTableLists.length == 0) {
      this.$message.error("请先勾选构件！")
    }else {
      this.dialogRatioValue = true
    }
  }

  private ratioOk() {
    this.checkTableLists.forEach(e => {
      e.ratio = this.ratioValue / 100
    });
    this.dialogRatioValue = false
  }

  private submitRecord() {
    if(this.cumulativeReportRatio == 1) {
      this.$message.error("累计填报比例为100%时，无法修改")
    }else {
      const arr = this.gridData.map((item=>{
        return {mbs: item.mbs, ratio: item.ratio * 100}
      }))
      const scheduleReportAddCmd =
      {
          projectCode: this.projectCode,
          reportBsId: this.reportBsId,
          scheduleReportMbsCmdList: arr,
      }
      if(this.submitText == "提交") {
        getAddMBSList(scheduleReportAddCmd).then(res=>{
          if(res.data?.flag) {
        ...
        this.scheduleReportMbsCmdList = arr
      }
    }
  }
  ...
  private selectChange(val) {
    this.reportBsId = val
    getReportMBSInfo(this.projectCode, this.projectConfig?.projectName, val).then(res => {
      this.gridData = res.data
    })
  }

  private onQuantitiesBlur() {
    // @ts-ignore
    this.outputValue = this.questList.selectRow?.planDetailUnitPrice * this.Quantities
    if (this.questList?.selectRow?.reportDetailMoney) {
      // @ts-ignore
      this.inputValue = (((Number(this.outputValue) + this.questList?.selectRow?.reportDetailMoney as any) / this.questList?.selectRow?.planDetailMoney) * 100).toFixed(2)
    } else {
      // @ts-ignore
      this.inputValue = (Number(this.outputValue) / this.questList?.selectRow?.planDetailMoney * 100).toFixed(2)
    }
  }

  private onBlur(val) {
    // @ts-ignore
    this.Quantities = (this.outputValue / this.questList.selectRow?.planDetailUnitPrice).toFixed(2)
    if (this.questList?.selectRow?.reportDetailMoney) {
      // @ts-ignore
      this.inputValue = (((Number(val.target._value) + this.questList?.selectRow?.reportDetailMoney as any) / this.questList?.selectRow?.planDetailMoney) * 100).toFixed(2)
    } else {
      // @ts-ignore
      this.inputValue = (Number(val.target._value) / this.questList?.selectRow?.planDetailMoney * 100).toFixed(2)
    }
  }
    ...
    Api.getWBSTreeV3({
      ...
      if (res.data) {
        ...
        if (firstLoad) {
          ...
          // });
          this.$nextTick().then(() => {
            MainTree?.setTreeExpand([this.progressCodeTree.source[0]],true);
          });
        }
      }
    })
  ...
  private isAllExpand(){
    this.isAllExpandShow = !this.isAllExpandShow
    if(this.isAllExpandShow) {
      this.MainTree?.setAllTreeExpand(true)
    }else {
      this.$nextTick().then(() => {
        this.MainTree?.setAllTreeExpand(false)
        this.MainTree?.setTreeExpand([this.progressCodeTree.source[0]],true);
      });
    }
  }
  ...
  private toDetails({column, row, $index}): JSX.Element {
    return (
      <span style="cursor: pointer" onClick={e => this.toDetailed(row)}>查看</span>
    )
  }

  private toDetailed(val){
    this.dialogTableRecord = true
    getReportMBSList(this.projectCode, this.projectConfig?.projectName, val.wbsId, val.mbs).then(res=>{
      res.data.forEach(e => {
        e.reportDate = e.reportDate?.split("T")[0]
        e.ratio = e.ratio * 100 + "%"
      });
  ...
  //查找父节点
  private familyTree (arr1, id) {
    const tempArr = []
    const forFn = function (arr, id1) {
      for (let i = 0; i < arr.length; i++) {
        const leafListArr = arr[i]
        if (leafListArr.id === id1) {
          tempArr.push(leafListArr as never)
          forFn(arr1, leafListArr.th04aPlandetailFk)
          break
        } else {
          if (leafListArr.children) {
            forFn(leafListArr.children, id1)
          }
        }
      }
    }
  ...
  private refreshProgressCodeTree(parentId?: string, firstLoad?: boolean) {
    ...
    Api.getWBSTreeV3({
      ...
      if (res.data) {
        ...
        if (firstLoad) {
          this.$nextTick().then(() => {
            if(this.progressCodeTree?.selectRow?.id && this.progressCodeTree?.source) {
              const arr = this.familyTree(this.progressCodeTree.source,this.progressCodeTree.selectRow.id)
              const asd = arr.reverse()
              MainTree?.setTreeExpand(asd,true);
            }
            this.getQuestList();
          });
        }
  ...
  private renderDetailed(row: Type.WBSTreeV2, locale: typeof ZhCNEx): JSX.Element {
    let vNode: JSX.Element = <span />;
    vNode = <span style="color: #40a9ff; cursor: pointer;" onClick={() => this.ToDetailed(row,"")}>设置</span>;
    return vNode;
  }

  private ToDetailed(val,text) {
    if(text == "构件信息") {
      this.submitText = "确定"
      this.disabled = true
    }else {
    ...
    this.cumulativeReportRatio = val.cumulativeReportRatio
    Api.getRecordWithId({
      id: val.id,
      ...
      if(res.data) {
        res.data.forEach(e => {
          e.reportDate = e.reportDate?.split("T")[0]
          //@ts-ignore
          e.reportRatio = e.reportRatio * 100
        });
        this.options = res.data.reverse()
        if(this.options) {
          this.selectValue = this.options[0].id
          this.reportBsId = this.options[0].id
        }
      }
      getReportMBSInfo(this.projectCode, this.projectConfig?.projectName, this.options[0].id).then(ress => {
        this.gridData = ress.data
      })
    })
  ...
  private questListColumns(locale?: typeof ZhCNEx): Array<Type.TableColumn<Type.WBSTreeV2>> {
    ...
    return [
      ...
      },
      {
        ...
        width: 80,
        align: 'center',
        customRender: (r) => this.renderDetailed(r as Type.WBSTreeV2, locale)
      },
    ]
  }
  ...
  private detailSubmit() {
    ...
    if (k.test(inputValue + '%')) {
      ...
      if(this.scheduleReportMbsCmdList.length == 0) {
        Api.submitFillDetail({
          idList: this.submitData.ids,
          projectCode: projectCode as string,
          ratio: inputValue,
        ...
        Api.submitFillDetail({
          ...
          ratio: inputValue,
          reportBsId: this.reportBsId,
          scheduleReportMbsCmdList: this.scheduleReportMbsCmdList,
          reportDate: moment(reportDate).format('YYYY-MM-DD')
        }).then(res => {
          removeTask(detailSubmit);
          if (res.errcode !== 0) return this.$message.error(res.errmsg as string);
