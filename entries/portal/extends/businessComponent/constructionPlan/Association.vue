    ...
    <div class="MBS">
      ...
      <div class="table">
        ...
        <vxe-table
          ...
          :checkboxConfig="{ labelField: 'bs' }"
          @checkbox-change="selectChangeEvent"
        >
          <vxe-column type="checkbox" title="MBS编码" treeNode></vxe-column>
          <vxe-column field="planDetailName" title="构件名称"></vxe-column>
        </vxe-table>
        <vxe-table
          v-if="!relation"
          class="mytable-scrollbar"
          ...
          :checkboxConfig="{ labelField: 'codeValue', checkRowKeys: checkRowKeys }"
          @checkbox-change="selectChangeEvent"
        >
          <vxe-column type="checkbox" title="MBS编码" treeNode></vxe-column>
          <vxe-column field="taskName" title="名称"></vxe-column>
        </vxe-table>
      </div>
    </div>
    <a-modal
...
import {getMBSBottom, getTreeChildList, noWBSLeafList, refreshWBSList2} from "../../service/api";
  ...
  methods: {
    ...
    handleAddOk() {
      if (this.addTitle == "新增子节点") {
        this.form.th04aPlandetailFk = this.wbsId;
      }
    ...
    addSonClick() {
      if (!this.wbsId) return this.$message.error("请先选择wbs编码");
      this.addTitle = "新增子节点";
      this.form = {
        id: "",
        Name: "",
    ...
    handleOk() {
      ...
      refreshWBSList2({ list: this.fixLists, projectCode: this.projectCode }).then((ress) => {
        this.visible = false;
    ...
    deleteClick() {
      ...
      refreshWBSList2({ list: [], projectCode: this.projectCode, listRemove: idLists }).then(
        (res) => {
          if (res.errcode == 0) {
            this.listRemove = [];
            this.getinitTable();
    ...
    onSearchMbs(val) {
      axios
        .get(`${env.apiHost}/api/code_mbs/top_tree_list`, {
          params: {
            projectCode: this.projectCode,
            wbsId: "",
            planId: this.schedulePlanId,
    ...
    getMbsTree(val) {
      axios
        ...
        .then((res) => {
          ...
          console.log(this.expandedMbsKeys);
          this.tableDataMbs = res.data;
          res.data?.forEach((e) => {
            if (e.mark) {
    ...
    ConfirmClick() {
      ...
      this.isCheckedLists.forEach((e) => {
        this.isCheckedLists.forEach((ele) => {
          if (ele.parentId == e.id) {
            qwer.push(ele);
          }
        });
      });
      const Afterscreening = this.isCheckedLists.filter((v) => qwer.every((val) => val.id != v.id));
      var arrsss = [];
      Afterscreening.forEach((e) => {
        if (e.childCount == 0) {
          e.bs = e.codeValue;
          e.id = Utils.uuid();
          e.wbs = e.codeValue;
          e.planDetailName = e.taskName;
          ...
          e.th04aPlandetailFk = this.wbsId;
          e.ta01CodeFk = e.id;
          e.Name = e.taskName;
          e.codeType = "MBS";
          e.state = "add";
          ...
          const arrs = [];
          arrs.push(e);
          refreshWBSList2({ list: arrs, projectCode: this.projectCode }).then((ress) => {
            this.getinitTable();
          });
        } else {
          getMBSBottom({ projectCode: this.projectCode, currentId: e.id }).then((res) => {
            arrsss = arrsss.concat(res.data);
            const newselects = [...new Set(arrsss)];
            newselects?.forEach((item) => {
              item.bs = item.codeValue;
              item.id = Utils.uuid();
              item.wbs = item.codeValue;
              item.planDetailName = item.taskName;
              item.planDetailStart = this.planDetailStart;
              item.planDetailEnd = this.planDetailEnd;
              // item.planDetailPeriod = this.planDetailPeriod;
              item.planDetailMoney = this.planDetailMoney;
              item.th04aPlanschemeFk = this.schedulePlanId;
              item.th04aPlandetailFk = this.wbsId;
              item.ta01CodeFk = item.id;
              item.Name = item.taskName;
              item.codeType = "MBS";
              item.state = "add";
              item.Critical = 0;
              item.warningProportion = 1;
            });
            refreshWBSList2({ list: newselects, projectCode: this.projectCode }).then((ress) => {
              if (ress.errcode == 0) return this.getinitTable();
            });
          });
        }
