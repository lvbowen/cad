...
import WBSCodeManage from "../WBSCodeManage/WBSCodeManage";
...
export default class ConstructionPlan extends Vue {
  ...
  private renderWarningProportion(cell, projectInstance): string {
    const value = cell.record[cell.field] || 1;
  ...
  private saveGantt(planId: string, cb?: Function) {
    ...
    // );
    // @ts-ignore
    realGanttData?.Tasks[0]?.th04aPlanschemeFk=planId
    console.log('realGanttData?.Tasks',realGanttData?.Tasks[0])
    refreshWBSList({
  ...
  private fileInput(e) {
    const files = e.target.files,{projectCode, planId, year, quarter, month, debounceGetWBSData, basicInfo} = this;
    console.log(files);
    
    if (files && files[0]) {
      const file = files[0];
      const fileFormat = file.name.slice(file.name.lastIndexOf(".") + 1).toLowerCase();
      if (["xlsx", "xls"].indexOf(fileFormat) === -1) return this.$message.error("请上传Excel文件!");
      ...
      if(this.isLeaf) {
        formData.append("wbsId", this.wbsId as string);
      }
      ...
      importWBSData(formData).then((res) => {
  ...
  private importWBSData() {
    ...
    if(this.businessState == "审核中" || this.businessState == "已审核") {
      ...
      if(this.wbsId) {
        this.isConfirm = true;
        this.ImportNode = "确认在该编码下导入吗？"
      }else if(this.ganttData.Tasks.length == 0){
        this.FileInput?.click();
      }else {
        this.$message.warn("请先选择编码！");
      }
    }
  }
  ...
  private ImportCancel() {
    this.isConfirm = false
  }

  private exportWBSData() {
  ...
  private downloadTemplate() {
    ...
    getProgressBSTemplate().then((res) => {
      if (res.errcode !== 0) {
  ...
  private ganttCellClick(val){
    console.log(val.record);
    if(val.record.children.length > 0) {
      this.isLeaf = false
    }else {
      this.isLeaf = true
    }
    this.codeType = val.record.codeType
    this.wbsId = val.record.id
  }
  ...
  mounted() {
    this.ganttOptions = [
      ...
      },
      {
        icon: "IImport",
        name: "导入",
        event: () => this.importWBSData(),
      },
      {
        icon: "IExport",
        name: "导出",
        event: () => this.exportWBSData(),
      },
      {
        icon: "save",
        name: "保存关键线路",
        ...
        <article class={Class.ganttPanel}>
          ...
          <main class={Class.ganttContainer}>
            ...
            <Gantt
              ...
              ganttCellClick={this.ganttCellClick}
              preTaskFnCollection={{
                get: getPreTaskArr,
                add: addPreTaskArr,
            ...
            <Modal onOk={this.ImportOk} onCancel={this.ImportCancel} title="导入" visible={this.isConfirm}>
              <span>{ this.ImportNode }</span>
            </Modal>
          </main>
        </article>
      </article>
