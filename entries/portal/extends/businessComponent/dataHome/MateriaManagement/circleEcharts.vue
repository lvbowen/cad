...
export default class pieEcharts extends Vue {
  @Prop() id!: string;
  @Prop() chartData!: { data: any, isLabelLineShow: boolean, midText: string, unitName: string, legend: Array<any>,center: Array<any>, radius: Array<any>,length: number,length2:number,orient: string, bottom: number,xlegend:string,fontSize: number,legendType?:string};
  @Prop({required:false,default:()=>true}) isShowTitle!:boolean;
  ...
  getOption(angle, dataChart, unitName) {
          ...
          normal: {
            label: {
              ...
              formatter: function (params) {
                if (params.name !== '') {
                  return '\n' + params.name + '：' +  (params.value / 100 * total).toFixed(0) + unitName;
                } else {
                  return '';
                }
              },
            ...
            labelLine: {
              // length: this.chartData.length || 35,
    ...
    return {
      ...
      legend: {
        type: this.chartData.legendType || '',
        icon: "circle",
      ...
      graphic: {
        ...
        top: "center",
        style: {
          text: this.isShowTitle? this.chartData.midText + "\n" + total:"",
          textAlign: "center",
          fill: "#00FFFC",
