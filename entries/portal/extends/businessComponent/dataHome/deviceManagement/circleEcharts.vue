...
export default class pieEcharts extends Vue {
  @Prop() id!: string;
  @Prop() chartData!: { data: any,isLabelLineShow: boolean, midText: string, unitName: string, legend: Array<any>, center: Array<any>, radius: Array<any>, length: number, length2: number, orient: string, bottom: number, xlegend: string, fontSize: number, legendShow: boolean, noTotal: boolean, color: string[] };
  ...
  getOption(angle, dataChart, unitName) {
    ...
    return {
      ...
      graphic: {
        ...
        top: "center",
        style: {
          text: this.chartData.midText + '\n' + (this.chartData.noTotal ? '': String(total) ),
          textAlign: "center",
          fill: "#00FFFC",
