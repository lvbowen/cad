...
import {getEnvironmentAndAir,getEnvironmentDataTrend,getProjectCityWeather,getEnvironmentDataPage} from "./index";
@Component({
...
export default class EnvironmentalMonitoring extends Vue{
  @Prop({required:true})
  projectName!: string;
  @Prop({required:true})
  treeId!: string;
  ...
  tableColumns:{key:string,dataIndex:string,title:string,align?:string,width?: number,scopedSlots?: object,customRender?:Function}[]=[
    // {key:"deviceTime",dataIndex:"deviceTime",title:"日期",align:"center",customRender:(text)=>{return this.formatDate(text)}},
    // {key:"temperature",dataIndex:"temperature",title:`温度(${this.htmlDecode("&#8451")})`,align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"humidity",dataIndex:"humidity",title:"湿度(%RH)",align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"windSpeed",dataIndex:"windSpeed",title:"风速(m/s)",align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"windDirection",dataIndex:"windDirection",title:"风向",align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"noise",dataIndex:"noise",title:"噪音(dB)",align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"airPollutionIndex",dataIndex:"airPollutionIndex",title:"空气质量",align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"pmTwoPiontFive",dataIndex:"pmTwoPiontFive",title:`PM2.5(μg/m${this.htmlDecode("&sup3")})`,align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"pmTen",dataIndex:"pmTen",title:`PM10(μg/m${this.htmlDecode("&sup3")})`,align:"center",customRender:(text)=>{return this.formatData(text)}},
    // {key:"tsp",dataIndex:"tsp",title:`TSP(μg/m${this.htmlDecode("&sup3")})`,align:"center",customRender:(text)=>{return this.formatData(text)}},
  ]
  ...
  mounted(){
    ...
    ,getEnvironmentDataPage(this.projectCode, this.projectName,1,this.pageSize,this.selectDataValue)]).then(values=>{
      this.isLoad=false;
  ...
  handleTableChange(pagination, filters, sorter){
    Promise.allSettled([getEnvironmentDataPage(this.projectCode, this.projectName,pagination.current,this.pageSize,this.selectDataValue)]).then(values=>{
  ...
  onDateChange(date: moment.Moment | string, dateString: string){
    ...
    Promise.allSettled([getEnvironmentDataPage(this.projectCode, this.projectName,1,this.pageSize,this.selectDataValue)]).then(values=>{
      this.getEnvironmentDataPage(values[0]);
    })
    this.getEnvironmentDataTrend();
  }
