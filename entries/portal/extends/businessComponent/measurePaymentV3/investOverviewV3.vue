  ...
  <div class="main">
    ...
    <a-card class="bottom_div">
      ...
      >
        ...
        <template v-slot:finishPercent="record">
  ...
  editTree(data, key, flag, value) {
    if (data.children?.length > 0) {
      for (let i = 0; i < data.children.length; i++) {
        if (flag) break;
        if (data.children[i].id !== key) return this.editTree(data.children[i], key, flag, value);
  ...
  getCBSByPeriod(period) {
    ...
    Api.getCBSByPeriodV3({
      ...
      .then(res => {
        ...
        if (res.data.length === 0) return this.$message.warn('本计量周期为空')
    ...
    Api.getLatestCBSV3({
      ...
      .then(res => {
        ...
        if (this.tableData.length > 0) {
          this.getCbsStatics(this.tableData[0].ccbs as string)
        }
    ...
    return {
      on: {
        click: () => {
          ...
          this.getCbsStatics(record.ccbs)
        },
      }
    };
  }
