<template>
  <div class="main">
    <a-card title="工程量清单方案" class="box" :bodyStyle="bodyStyle">
      ...
      <a-button slot="extra" type="primary" @click="saveBtn">保存</a-button>
      <a-button slot="extra" type="primary" @click="curBtn">设为当前版本</a-button>
      <a-button slot="extra" @click="delBtn" type="danger">删除
        <a-popconfirm
          title="确认删除此行？"
      ...
      >
        ...
        <template slot="schemeName" slot-scope="text, record">
          <editable-cell :text="text" @change="onCellChange(record.id, 'schemeName', $event)"/>
        </template>
      </a-table>
    </a-card>
...
export default class allProtect extends Vue {
  ...
  currentRow: null | { [propsName: string]: string | number } = null; //点击表格行，保存当前行数据
  ...
  curBtn() {
    ...
    this.tableData.forEach((v, i) => {
      if (this.currentRow?.['id'] === v.id) {
        ...
        this.setCurrentVersion(v.id, this.currentRow?.['contractNum']);
      }
    });
  }
  ...
  modalOk() {
    // 判断有没有选择合同
    if (this.currentRow?.['contractName'] != "") {
  ...
  saveBtn() {
    ...
    if (!this.currentRow?.['contractName'] || this.currentRow?.['contractName'] == "") return this.$message.warn("请选择合同！");
  ...
  setCurrentVersion(v, contractNum) {
    Api.setCurrentVersion({
      projectCode: this.projectCode ?? '',
      schemaId: v as string,
      contractNum: contractNum as string
    }).then(res => {
      if (res.errcode !== 0) return this.$message.warn(res.errmsg as string);
      this.init()
    })
