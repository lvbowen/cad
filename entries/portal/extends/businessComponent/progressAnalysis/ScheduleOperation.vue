<template>
  <div class="ScheduleOperation">
    ...
    <div class="outputValue">
      ...
      <!-- 进度条 -->
      <div class="outputValue_progress_new">
        <h4>进度产值完成情况</h4>
        <h5>总产值</h5>
      ...
      <div class="outputValue_progress">
        ...
        <div class="outputValueProportion">
          <!-- <el-carousel height="220px" :interval="20000" indicatorPosition= "none"> -->
            <!-- <el-carousel-item v-for="item in 2" :key="item"> -->
              <line-echarts :id="'outputValueLinechat'" :chartData="outputValueData"></line-echarts>
              <!-- <bar-echarts2 v-if="item == 2" :id="'BarEcharts2'" :chartData="barData2"></bar-echarts2> -->
            <!-- </el-carousel-item>
          </el-carousel> -->
        </div>
      </div>
    ...
    <div class="progressGannt">
      ...
      <Gantt
        ...
        ref="GanttRef"
      />
      <div v-if="!isGanttShow" style="width: 100%;height: 100%">
        <bar-echarts id="BarEcharts" :chartData="barData"></bar-echarts>
      </div>
    </div>
  </div>
</template>
...
import BarEcharts from "../ManageViews/components/BarEcharts.vue";
import env from "@/config/env";
import {Gantt, GanttClass} from "@ctesi/component";
import * as Type from "../../type";
import {getTreeList, getSchedulePlanDetailList} from "../../service/api";
import Utils from "../../utils";
import omit from "omit.js";
import Input from "element-ui/lib/input";
import moment from "moment";
import {getdeviation, getscheduleAnalyse, getscheduleAnalyseState,getMonthPlanReport} from "./serve/index.js";
...
import BarEcharts2 from "../BIMPlatform/Echarts/BarEcharts.vue"
...
@Component({
  ...
  components: {
    ElProgress: Progress, PieCharts, LineEcharts, BarEcharts,BarEcharts2,
    Gantt, ElInput: Input, CheckBox: CheckBox, CheckBoxGroup: CheckBoxGroup,
...
export default class scheduleAnalysis extends Vue {
  ...
  barData: any = {
    ...
    dataAll:[],
    yColor: 'black',
    seriesColor0: "rgb(3,204,240)",
    seriesColor1: "rgb(79,87,238)",
    grid: {
      top: "10%",
      left: "8%",
      right: "5%",
  ...
  private ganttConditionChange(value: Array<string>): void {
    ...
    if (value.length > 0) {
      this.isGanttShow = false;
      this.getSchedulePlanDetailList();
    } else {
      this.isGanttShow = true;
    }
    console.log('ganttConditionChange', value)
  }
  ...
  getSchedulePlanDetailList(planId?: string, year?: number, season?: number, month?: number, planDetailName?: string, progressState?: string) {
    const {ganttConditionCheckType} = this;
    const requestData = {
    ...
    getSchedulePlanDetailList(omit(requestData, omitPropertyArray)).then((res) => {
      if (res.errcode !== 0) return this.$message.error(res.errmsg as string);
      // res.data && wbsDataAdapter(res.data as unknown as Array<bizGanttData>);
      console.log('getSchedulePlanDetailList', res.data)
      res.data?.forEach((item) => {
        this.barData.xAxis.push(item.planDetailName);
        this.barData.yAxis.push((item.completeRatio ?? 0) * 100);
        this.barData.series.push((item.completeRatio ?? 0) * 100);
        this.barData.dataAll.push({
          planDetailMoney:item.planDetailMoney,
          //@ts-ignore
          reportDetailMoney:item.reportDetailMoney,
          planDetailName:item.planDetailName,
        });
      })
    });
  }
  ...
  getinit() {
    ...
    getscheduleAnalyse(this.projectCode, "", this.projectName).then((res) => {
      if (res.data) {
        console.log('res.data.nodeAnalyseValue.totalValue', typeof res.data.nodeAnalyseValue.totalValue)
        //@ts-ignore
        this.totalOutputValue = (res.data.nodeAnalyseValue.totalValue / 10000).toFixed(2) as number;
        //@ts-ignore
        this.completeValue = (res.data.nodeAnalyseValue.completeValue / 10000).toFixed(2) as number;
        //@ts-ignore
        this.completeRatio = (res.data.nodeAnalyseValue.completeRatio * 100).toFixed(2) as number;
        //@ts-ignore
        this.surplusValue = (res.data.nodeAnalyseValue.leftValue / 10000).toFixed(2) as number;
        //@ts-ignore
        this.leftRatio = (res.data.nodeAnalyseValue.leftRatio * 100).toFixed(2) as number;
      }
    })
  }
  ...
  mounted() {
    ...
    this.getinit();
    this.getdeviation();
    getMonthPlanReport(this.projectCode,this.projectName).then(res=>{
      res.data?.forEach(e => {
        this.barData2.xAxis.push(e.monthDate)
        this.barData2.yAxis1.push(e.monthPlanMoney)
        this.barData2.yAxis2.push(e.monthReportMoney)
      });
    })
  }
