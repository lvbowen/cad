...
import Class from './codeMbs.module.less';
...
import Stars  from '../../../src/assets/extends/icon/stars.png';
import {MBSTable, MountQBSTable} from "../../type";
...
interface MountNodes {
  dataSource: MountQBSTable[],
  loading: boolean
}

interface JournalModal {
  key: number,
  journalModalVisible: boolean,
  dataSource: Type.QBSLog,
  loading: boolean,
  pageNum: number,
  pageSize: number,
  type: string,
...
export default class QualityModule extends Vue {
  ...
  private mbsBindStatus: number = 0;
  //导入
  private importType: string = '';
  private isLoadQbsTree: boolean = false;
  private structureTypeModalVisible: boolean = false;
  ...
  private cascaderKey: number = 0;
  private levelOption: {key: number, value: number, text: string}[] = [];
  private importTableColumns(locale?: typeof ZhCNEx) {
    if ( !locale ) return [];
    const defaultColumn = [
      {
        title: '部位码',
      ...
      },
      {
        ...
        editable: true,
        customRender: (t,r)=> {
          let upData:Type.QualityTable[] = [];
          if(this.mainTable.selectRow) {
            const a = JSON.parse(JSON.stringify(this.mainTable.dataSource));
            this.defaultSelected(a,'childs','id',this.mainTable.selectRow.id,this.importTable.dataSource)
            upData = a
          }else {
            upData = this.importTable.dataSource
          }
          //划分类别-具体部位以下节点都不可编辑
          const upFlag:boolean = !r.parentId ? true :this.validateUpLevel(upData,'childs','level',QBSLevel.INSPECTION_LOT,r.parentId);
          // console.log(upFlag,r.qbsCode)
          return !upFlag?'':<Select v-model={t} onChange={(val)=> this.editNodeLevel(val,r)}>
            {r.levelOption.map((item)=> {
              return <Select.Option key={ item.key } value={ item.value }>{ item.text }</Select.Option>
            })}
          </Select>
          // return !upFlag?'':<Select v-model={t} onChange={(val)=> this.editNodeLevel(val,r)}>
          //   <Select.Option key={ 0 } value={ 0 }>
          //     { locale?.Engineering.UNIT_PRO_PROJECT}
          //   </Select.Option>
          //   <Select.Option key={ 1 } value={ 1 }>
          //     { locale?.Engineering.UNIT_PROJECT }
          //   </Select.Option>
          //   <Select.Option key={ 2 } value={ 2 }>
          //     { locale?.Engineering.SUB_UNIT_PROJECT }
          //   </Select.Option>
          //   <Select.Option key={ 3 } value={ 3 }>
          //     { locale?.Engineering.PARTITION_PROJECT }
          //   </Select.Option>
          //   <Select.Option key={ 4 } value={ 4 }>
          //     { locale?.Engineering.SUB_PARTITION_PROJECT }
          //   </Select.Option>
          //   <Select.Option key={ 5 } value={ 5 }>
          //     { locale?.Engineering.ITEM_PROJECT }
          //   </Select.Option>
          //   <Select.Option key={ 6 } value={ 6 }>
          //     { locale?.Engineering.INSPECTION_LOT }
          //   </Select.Option>
          // </Select>
        }
        // width: 150
      },
      {
        ...
        customRender: (t,r)=> {
          const a:string[] = [];
          Utils.deepFind(this.importTable.structionTypeOption,item => {
            if(item.id === t) {
              a.push(item.parentId,item.id)
            }
            return false
          },'childs')
          return [QBSLevel.ITEM_PROJECT,QBSLevel.INSPECTION_LOT].includes(r.level)?<Cascader
            key={this.cascaderKey}
            placeholder={'请选择'}
            options={this.importTable.structionTypeOption}
            fieldNames={{label: 'name', value: 'id', children: 'childs' }}
            onChange={(val)=> {this.editNodeStructionType(val,r)}}
            defaultValue={a}
          ></Cascader>:''
        },
        width: 200
  ...
  private editNodeLevel(val,r) {
    ...
    const upFlag:boolean = !r.parentId ? true :this.validateUpLevel(upData,'childs','level',val,r.parentId,);
    if(!upFlag) return this.$message.error('节点上级存在底层级标签节点')
    const downFlag:boolean = r.childs && this.validateDownLevel(r.childs,'childs','level',val)
    if(!downFlag) return this.$message.error('节点下级存在高层级标签节点')
    //赋值
    Utils.deepFind(this.importTable.dataSource,item=> {
      if(item.id == r.id) {
        if(val===QBSLevel.ITEM_PROJECT&&item.childs&&item.childs.length) {
          item.childs.map((subItem)=> {
            subItem.level = QBSLevel.INSPECTION_LOT;
            subItem.modelTypeId = item.modelTypeId;
          })
        }else if(r.level===QBSLevel.ITEM_PROJECT){ //最开始选择分项工程，后来改变为其他划分列别，其子级需要恢复初始状态
          item.childs.map((subItem)=> {
            subItem.level = null;
            subItem.modelTypeId = '';
            this.cascaderKey++
        ...
        item.level = val;
        //导入设置时设置选择上一级后，下拉框只能选择下一级的选项:注意：level取值与数组index一一对应
        if(item.childs && Array.isArray(item.childs)) {
          Utils.deepFind(item.childs,(i)=> {
            i.levelOption = this.levelOption.slice(val+1,this.levelOption.length);
            return false
          },'childs')
        }
      }
  ...
  //0,1,2,3,……,null
  //节点上级存在底层级标签节点 - 向上递归
  private validateUpLevel(arr:Type.QualityTable[], attChildren:string = 'childs',key:string,value:number,parentId:string) {
    let flag = true;
    const parentRow:Type.QualityTable[] = this.getRow(arr, attChildren, 'id', parentId)
    if ((typeof(parentRow[0][key])==='number' && parentRow[0][key]>=value)) {
      flag=false
    }else if(parentRow[0][key]===null && parentRow[0].parentId){
      const p = this.validateUpLevel(arr,attChildren,key,value,parentRow[0].parentId);
      if(!p) {
        flag=false
      }
    }
  ...
  private getRow(arr:Type.QualityTable[], attChildren:string = 'childs',key:string,value:any) {
    let finalArr = [];
    for (let i=0;i<arr.length;i++) {
      ...
      if (arr[i][attChildren]&&arr[i][attChildren].length) {
        const p = this.getRow(arr[i][attChildren], attChildren, key, value);
        finalArr = finalArr.concat(p)
      }
    }
    return finalArr;
  }
  private defaultSelected(arr:Type.QualityTable[], attChildren = 'children',key:string,value:string,data:Type.QualityTable[]) {
    for(let i=0;i<arr.length;i++) {
      if(arr[i][key]===value){
        arr[i][attChildren] = data;
       break
      }
      if(arr[i][attChildren] && arr[i][attChildren].length) {
        this.defaultSelected(arr[i][attChildren], attChildren,key,value,data);
      }
    }
  }
  //节点下级存在高层级标签节点 - 向下递归
  private validateDownLevel(arr:Type.QualityTable[], attChildren:string = 'childs',key:string,value:number) {
    let flag = true;
    for(let i=0;i<arr.length;i++) {
      if(typeof(arr[i][key])==='number' && arr[i][key]<=value) {
        flag = false;
        break;
      }
      if(arr[i][attChildren]&&arr[i][attChildren].length) {
        const is:boolean = this.validateDownLevel(arr[i][attChildren],attChildren,key,value)
        if(!is) {
          flag = false;
          break;
        }
  ...
  private editNodeStructionType(val:string[],r) {
    Utils.deepFind(this.importTable.dataSource,item=> {
      if(item.id == r.id) {
        item.modelTypeId = val[1] //只有两级
        if(r.level===QBSLevel.ITEM_PROJECT&&item.childs&&item.childs.length) {
          item.childs.map((subItem)=> {
            subItem.modelTypeId = val[1]
          })
          this.cascaderKey++
        }
      }
  ...
  //操作日志
  private journalModal:JournalModal = {
    key: 1,
    ...
    keyWord: '',
    selectKeys: [],
    typeOption: ['新增节点','删除节点','修改节点','全量导入','增量导入','新增工序', '删除工序','修改工序'],
    startTime:'',
    endTime:'',
    timeAscFlag: false
  }
  private openJournalModal() {
    this.journalModal.selectKeys = [];
    this.journalModal.journalModalVisible = true;
    this.getQBSLog();
  }
  private getQBSLog() {
    this.journalModal.loading = true;
    Api.QBSLog({
      appCode: this.projectCode??'',
      projectName: this.projectConfig?.projectName??'',
      pageNum: this.journalModal.pageNum,
      pageSize: this.journalModal.pageSize,
      keyWord: this.journalModal.keyWord,
      type: this.journalModal.type,
      timeAscFlag: this.journalModal.timeAscFlag,
      startTime:  this.journalModal.startTime?moment(this.journalModal.startTime).format( 'YYYY-MM-DD' ):'',
      endTime:this.journalModal.endTime?moment(this.journalModal.endTime).format( 'YYYY-MM-DD' ):''
    }).then((res)=> {
      if(res.errcode!==0) return this.$message.error(res.errmsg as string)
      //@ts-ignore
      this.journalModal.dataSource = res.data;
    }).finally(()=> {
      this.journalModal.loading = false
    })
  }
  private qbsLogTableColumns(locale?: typeof ZhCNEx) {
    if ( !locale ) return [];
    const defaultColumn = [
      {
        title: '操作人',
  ...
  private paginationChange( page: number, pageSize: number ) {
    this.journalModal.pageNum = page;
    this.journalModal.pageSize = pageSize;
    this.openJournalModal();
  }
  private journalRowSelection(selectedRowKeys, selectedRows) {
    this.journalModal.selectKeys = selectedRowKeys;
  }
  private deleteLog() {
    if(!this.journalModal.selectKeys.length) return this.$message.warning('请至少选择一项!');
    Api.deleteQBSLog({
     ids: this.journalModal.selectKeys
    }).then((res)=> {
      if(res.errcode!==0) return this.$message.error(res.errmsg as string)
      this.openJournalModal();
    })
  ...
  private rangeStartPicker( val: Moment ) {
    return moment( val ).valueOf() > moment( this.journalModal.endTime ).valueOf();
  }
  private rangeEndPicker( val: Moment ) {
    return moment( this.journalModal.startTime ).valueOf() >= moment( val ).valueOf();
  }
  private changeTimeAscFlag(checked) {
    this.journalModal.timeAscFlag = checked;
    this.getQBSLog();
  }
  private resetJournalCondition() {
    this.journalModal.keyWord = '';
    this.journalModal.startTime = '';
    this.journalModal.endTime = '';
    this.journalModal.type = '';
    this.journalModal.timeAscFlag = false;
    this.journalModal.pageSize = 7;
    this.journalModal.pageNum = 1
  }
  private originalJournal() {
    this.resetJournalCondition();
    this.getQBSLog();
  }
  private cancelJournalModal() {
    this.resetJournalCondition();
    this.$nextTick(()=> {
      this.journalModal.journalModalVisible = false;
    })
  }
  scopedSlotsList() {
    const obj = {}
    obj['leval'] = (text, record, index) => {
      return <p>text</p>
    }
    return obj
  }
  ...
  mainTableSelectRowListener( val ) {
    console.log( 'val===>', val );
    console.log('data',this.mainTable.dataSource)
  ...
  private getMBSTableColumn( locale?: typeof ZhCNEx, checkEnable?: boolean ): Array<Type.TableColumn<any>> {
    ...
    const defaultColumn = [
      ...
      },
      {
        ...
        key: 'header_1',
        customRender: (t,r)=> {
          return <div class={Class.stars}><span>{r.taskName}</span>{r.bindQbsFlag?<img src={Stars}/>:''}</div>
        }
      }
    ];
    if ( !checkEnable ) {
  ...
  private fileInput( e ) {
    ...
    if ( files && files[0] ) {
      ...
      if(this.importType==='code_mbs'){
        formData.append( "projectCode", projectCode as string );
      }else {
        formData.append( "appCode", projectCode as string );
        this.mainTable.selectRow ? formData.append('parentQbsCode',this.mainTable.selectRow.qbsCode):''
      }
  ...
  private startUpload() {
    ...
    const api = this.importType==='code_mbs'?Api.importMBSTree:Api.importQBSTree
    api( this.uploadFile ).then( res => {
      this.uploadLoading = false;
      if ( res.errcode !== 0 ) {
        ...
        if(this.importType==='code_mbs') {
          ...
          this.structureTypeModalVisible = true;
          this.importTable.dataSource = res.data??[];
          this.levelOption = [
            {
              key:0,
              value:0,
              text: locale?.Engineering.UNIT_PRO_PROJECT??'UNIT_PROJECT'
            },
            {
              key:1,
              value:1,
              text: locale?.Engineering.UNIT_PROJECT??'单位工程'
            },
            {
              key:2,
              value:2,
              text: locale?.Engineering.SUB_UNIT_PROJECT??'子单位工程'
            },
            {
              key:3,
              value:3,
              text: locale?.Engineering.PARTITION_PROJECT??'分部工程'
            },
            {
              key:4,
              value:4,
              text: locale?.Engineering.SUB_PARTITION_PROJECT??'子分部工程'
            },
            {
              key:5,
              value:5,
              text: locale?.Engineering.ITEM_PROJECT??'分项工程'
            },
            {
              key:6,
              value:6,
              text: locale?.Engineering.INSPECTION_LOT??'具体部位'
            }
          ]
          Utils.deepFind(this.importTable.dataSource,(item)=> {
            //导入设置时设置选择上一级后，下拉框只能选择下一级的选项
            if(this.mainTable.selectRow) {
              item.levelOption = this.levelOption.slice(this.mainTable.selectRow.level+1,this.levelOption.length)
            }else{
              item.levelOption = this.levelOption;
            }
            return false
          },'childs')
          // this.initQBSImportModal();
 ...
 //导入数据编辑
  private getEditStructureTypeView() {
    this.importTable.loading = true
    const parmas = {
      appCode: this.projectCode??'',
      projectName: this.projectConfig?.projectName??'',
      nodes: this.importTable.dataSource,
      parentQbsCode: this.mainTable.selectRow?.qbsCode??''
    }
    Api.QBSTreeImportConfirm(parmas).then((res)=> {
      if(res.errcode!==0) return this.$message.error(res.errmsg as string);
      this.$message.success('导入成功');
      this.importTable.dataSource = [];

      this.initQBSImportModal();
      this.initMbsStateCollection();
      this.getQBSTree();
    }).finally(()=> {
      this.importTable.loading = false
      this.structureTypeModalVisible = false;
    })
  }

  private openImportModal(type:string) {
    this.importType = type;
    this.qbsImportModalVisible = true
  }
    ...
    return (
      <article class={ Class.main } ref={ 'MainContainer' }>
        ...
        <nav class={ Class.nav }>
          ...
          <Button type={'primary'} onClick={()=> this.openJournalModal()}>操作日志</Button>
          {/*{*/ }
          {/*  !editMode &&*/ }
          {/*  <Button*/ }
        ...
        <main class={ Class.mainContainer }>
          ...
          <SyncVisualTable
            ref={ 'MBSTree' }
            config={ {
              wrapperClass: mainTable.selectRow?Class.MBSTree:Class.hiddenMbsTree,
              expendAllNode: true,
              ...
              articleNavNodes: !mbsStateCollection.operation && [] || this.renderMBSMainNav(),
              mainTableConfig: {
                size: 'mini',
                align: 'center',
                stripe: false,
                ...
                cellDBClick: this.showRelevanceQbs,
              },
          ...
          <SyncVisualTable
            ...
            config={ {
              ...
              loading: mbsStateCollection.loading,
              articleNavNodes: this.renderMBSMainNav(),
              mainTableConfig: {
                size: 'mini',
                align: 'center',
                stripe: false,
              ...
              navNodes: this.renderMBSTreeNav(),
              lazy: false,
              columns:this.getMBSTableColumn( locale, mbsStateCollection.checkEnable ),
        ...
        >
          <article class={ Class.qbsModal }>
              ...
              <aside class={ Class.row }>
                <span>{ locale?.Engineering.qbsImportTips }
                  <span class={ Class.link }
                        onClick={ e => this.importType==='code_mbs'?window.open( '/api/api/code_mbs/data_template' ):window.open( '/api/api/quality/v2/qbs/excel/importTemplate' ) }>{ locale?.Engineering.qbsImportDownloadTemplate }</span>
                </span>
              </aside>
              <aside class={ Class.row }>
            ...
            <div class={ Class.footer }>
              ...
              <Popconfirm
                title={this.importType==='code_mbs' || (this.importType===''&&!mainTable.selectRow)?'导入数据将会覆盖原有数据，请确认？':`当前选择节点部位码是${mainTable.selectRow?.qbsCode}，将会在此节点下增量导入，请确认！`}
                onConfirm={this.startUpload}
              >
                <Button
                  type={ 'primary' }
                  loading={ this.uploadLoading }
                >
                  { this.importType==='code_mbs'?locale?.Common.Action.ok:locale?.Common.Action.next }
                </Button>
              </Popconfirm>
            </div>
          </article>
        ...
        <Modal
          closable={ false }
          visible={ this.structureTypeModalVisible }
          destroyOnClose={ true }
          maskClosable={ false }
          footer={ false }
          onCancel={ e => this.structureTypeModalVisible = false }
          width={ 1000 }
        >
          <article class={ Class.qbsModal }>
            <nav>
            ...
            <main class={Class.structureType}>
              <Table
                dataSource={importTable.dataSource}
                columns={this.importTableColumns(locale)}
                childrenColumnName={'childs'}
                pagination={false}
                loading={importTable.loading}
                defaultExpandAllRows={true}
                rowKey={'id'}
                bordered
              >
                {/*{<template slot={"level"} slot-scope={'text,scope'}>*/}
                {/*  <a onClick={(scope) => this.tableShow(scope)} href="javascript:;">查看</a>*/}
                {/*</template>}*/}
                {/*<template scopedSlots="text`" slot="level">*/}
                {/*  {text}*/}
                {/*</template>*/}

                {/*{<template slot={'level'}  slot-scope="{text,scope,index}">{(text,scope)=> { return '11111'}}</template>}*/}
              </Table>
            </main>
            <div class={Class.footer}>
              <Button class={Class.importBtn} onClick={()=> Utils.deepFind(this.importTable.dataSource,(i)=> {
                i.level = null;
                i.modelTypeId = '';
                return false
              },'childs')}>
                {locale?.Common.Action.reset}
              </Button>
              <Button
               onClick={()=> this.structureTypeModalVisible = false}
              >
                {locale?.Common.Action.cancel}
              </Button>
              <Button
                type={'primary'}
                loading={this.importTable.loading}
                onClick={()=> this.getEditStructureTypeView()}
              >
        ...
        <Modal
          closable={ false }
          visible={ this.mountNodesModalVisible }
          destroyOnClose={ true }
          maskClosable={ false }
          footer={ false }
          onCancel={ e => this.mountNodesModalVisible = false }
          width={ 700 }
        >
          <article class={ Class.qbsModal }>
            <nav>
              <span class={ Class.topDivision }/>
              <span>编辑挂载节点结构类型</span>
            </nav>
            <main class={Class.structureType}>
              <Table
                dataSource={this.mountTableNodes.dataSource}
                columns={this.mountTableColumns(locale)}
                childrenColumnName={'childs'}
                pagination={false}
                loading={this.mountTableNodes.loading}
                defaultExpandAllRows={true}
                rowKey={'codeValue'}
                bordered
              >
            ...
            <div class={Class.footer}>
              <Button
                onClick={()=> this.mountNodesModalVisible = false}
              >
                {locale?.Common.Action.cancel}
              </Button>
              <Button
          ...
          <article class={ Class.qbsModal }>
            ...
            <main class={ Class.qbsModalForm }>
              ...
              <aside class={ Class.row }>
                ...
                <span>{ locale?.Engineering.qbsCode }：</span>
              ...
              {
                this.qbsModal.edit && this.qbsModal.level===QBSLevel.INSPECTION_LOT&&
                [
                  <nav>
                    <i/>
            ...
            <div class={ Class.footer }>
              ...
              <Button onClick={ this.submitQBSModal } loading={this.qbsModal.loading}>{ locale?.Common.Action.ok }</Button>
            </div>
          </article>
        </Modal>
        <Modal
        ...
        <Modal
          key={this.journalModal.key}
          closable={ true }
          visible={ this.journalModal.journalModalVisible }
          destroyOnClose={ true }
          title={'操作日志'}
          maskClosable={ false }
          onCancel={ e => this.cancelJournalModal()}
          width={ 1000 }
        >
          <article class={ Class.qbsModal }>
            <div class={Class.searchType}>
              <Row>
                <Col span={4}>
                  <span class={Class.label}>关键字:</span>
                  <Input v-model={this.journalModal.keyWord}></Input>
                </Col>
                <Col span={5}>
                  <span  class={Class.label}>操作类型:</span>
                  <Select v-model={this.journalModal.type} onChange={ e => {
                    this.getQBSLog();
                  } }>
                    {this.journalModal.typeOption.map(item=> <Select.Option value={item}>{item}</Select.Option>)}
                  </Select>
                </Col>
                <Col span={10}>
                  <span class={Class.label}>操作时间:</span>
                  <DatePicker
                    v-model={ this.journalModal.startTime }
                    allowClear={ false }
                    disabledDate={ this.rangeStartPicker }
                  />
                  <span class={ Class.division }>-</span>
                  <DatePicker
                    v-model={ this.journalModal.endTime }
                    allowClear={ false }
                    disabledDate={ this.rangeEndPicker }
                  />
                </Col>
                <Col span={2} >
                  <Switch checkedChildren={'升序'} unCheckedChildren={'降序'} onChange={this.changeTimeAscFlag}/>
                </Col>
                <Col span={3}>
                  <Button type={'primary'} size={'small'} onClick={this.getQBSLog}>查询</Button>
                  <Button type={'primary'} size={'small'} onClick={this.originalJournal}>重置</Button>
                </Col>
              </Row>
            </div>
            <main class={Class.structureType}>
              <Table
                dataSource={this.journalModal.dataSource.hits}
                columns={this.qbsLogTableColumns(locale)}
                loading={this.journalModal.loading}
                rowKey={'id'}
                pagination={{
                  position: 'bottom',
                  pageSize: this.journalModal.pageSize,
                  total: this.journalModal.dataSource.totalHits,
                  onChange: this.paginationChange
                }}
                rowSelection={{
                  onChange:this.journalRowSelection,
                  selectedRowKeys: this.journalModal.selectKeys
                }}
              ></Table>
            </main>
          </article>
          <template slot={'footer'}>
            <Popconfirm  onConfirm={()=> this.deleteLog()}>
              <div slot={'title'}>确认删除吗？删除后不可恢复哦</div>
              <Button>删除</Button>
            </Popconfirm>
          </template>
        </Modal>
  ...
  private getTableColumn( locale?: typeof ZhCNEx, checkBoxEnable?: boolean ): Array<Type.TableColumn<any>> {
    ...
    const defaultColumns: Array<Type.TableColumn<any>> = [
      ...
      {
        title: locale.Engineering.qbsCode,
        align: 'left',
        dataIndex: 'qbsCode',
        ellipsis: true,
        key: 'header_0',
 ...
 //挂载节点编辑结构
  private mountNodesModalVisible: boolean = false;
  private mountTableNodes:MountNodes= {
    dataSource: [],
    loading: false
  }
  private openMountNodesModal() {
    ...
    //@ts-ignore
    this.mountTableNodes.dataSource = [
      {
        codeValue: this.mainTable.selectRow?.qbsCode??'',
        taskName: this.mainTable.selectRow?.qbsName??'',
        modelTypeId: this.mainTable.selectRow?.modelTypeId??'',
        level: QBSLevel.ITEM_PROJECT,
        childs:checkRows.map((item)=> {
          return {
    ...
    this.mountNodesModalVisible = true;
    console.log(this.mountNodesModalVisible,'mountNodesModalVisible',this.mountTableNodes)

  }
  private mountTableColumns(locale?: typeof ZhCNEx) {
    if ( !locale ) return [];
    const defaultColumn = [
      {
        title: '任务名称',
      ...
      },
      {
        ...
        width: 130,
        customRender: (t,r)=> {
          return r.level===QBSLevel.ITEM_PROJECT?locale?.Engineering.ITEM_PROJECT:r.level===QBSLevel.INSPECTION_LOT?locale?.Engineering.INSPECTION_LOT:''
        }
      },
      {
        title: '结构类型',
        ...
        customRender: (t,r)=> {
          ...
          return [QBSLevel.ITEM_PROJECT,QBSLevel.INSPECTION_LOT].includes(r.level)?<Cascader
            ...
            options={this.importTable.structionTypeOption}
            fieldNames={{label: 'name', value: 'id', children: 'childs' }}
            onChange={(val)=> {this.editMountNodes(val,r)}}
            defaultValue={a}
          ></Cascader>:''
        }
        // width: 150
  ...
  private editMountNodes(val:string[],r) {
    Utils.deepFind(this.mountTableNodes.dataSource,item=> {
      if(item.codeValue == r.codeValue) {
        item.modelTypeId = val[1] //只有两级
        if(r.level===QBSLevel.ITEM_PROJECT&&item.childs&&item.childs.length) {
          item.childs.map((subItem)=> {
            subItem.modelTypeId = val[1]
  ...
  private confirmMount() {
    ...
    this.mountTableNodes.loading = true
    Api.mountQBSTree( {
      appCode: projectCode as string,
      // @ts-ignore
      addNodes: this.mountTableNodes.dataSource[0].childs.map( item => {
        return {
          codeValue: item.codeValue,
          taskName: item.taskName,
      ...
      this.mountTableNodes.loading = false
      ...
      this.mountNodesModalVisible = false;
    })
  }
  ...
  private getJBSTree( nodeId?: string ) {
    ...
    return Api.queryJBSTree( {
      ...
      //导入数据所需结构类型数据
      this.importTable.structionTypeOption = [...qbsModal.projectOptions];
      Utils.deepFind(this.importTable.structionTypeOption,item=> {
        if(item.level===1) {
          item.childs = []
        }
  ...
  private editQBSTreeNode() {
    ...
    qbsModal.level = mainTable.selectRow.level === null ? undefined : mainTable.selectRow.level;
  ...
  private getMBSTree( qbsId?: string, mbsId?: string ) {
    ...
    if(this.mbsBindStatus!==0) {
      Object.assign(params,{ bind: this.mbsBindStatus === 1})
    }
    Api.getMBSTree(params).then( res => {
      this.mbsStateCollection.loading = false;
      if ( res.errcode !== 0 ) return this.$message.error( res.errmsg as string );
  ...
  private getRelevanceStatus(val) {
    this.mbsBindStatus = val;
    switch (val) {
      case 0:
        this.openMBSOpt();
       break;
      case 1:
        this.getHasRelevance();
       break;
      case 2:
        this.getNoRelevance();
        break;
      ...
      onOk() {
        return new Promise( resolve => {
          Api.removeQBSTreeNode( {
            ...
            if ( res.errcode !== 0 ) {
              errorModal(res.data?.title??'',res.data?.contents??[])
              resolve( {})
              // return $message.error( res.errmsg as string );
            }else{
  ...
  private errorModal(title:string,contents:string[]) {
    Modal.error({
      title: title,
      content: (
        contents.map((i)=>{
  ...
  private showRelevanceQbs({ row }: { row: Type.QualityTable }) {
    const locale =  this.locale;
    Api.getQbsListByMbsId({
      appCode: this.projectCode??'',
      projectName: this.projectConfig?.projectName??'',
      nodeId: row.id
    }).then((res)=> {
      ...
      if(res.data && res.data.length) {
        ...
        Modal.info({
          ...
          content: (
             <SyncVisualTable
               config={{
                 wrapperClass: Class.relevanceQbs,
                 locale: {
                   title: "选中构件部位码列表展示"
                 },
                 expendAllNode: false,
                 ...
                 dataSource: res.data,
                 columns:(this.getTableColumn( locale)).splice(0,2),
                 lazy: false
               }}
            />
            // res.data.map((i)=> {
            //   return <p style={{whiteSpace: 'nowrap'}}>{i.qbsName}</p>
            // })
          ),
          okText: '知道了'
        })
      }else {
        this.$message.warning('选中构件未进行关联设置！')
      }
    })
  }
  ...
  private renderQBSTreeNav() {
    ...
    >
      ...
      <Button
        onClick={ e => window.open( `/api/api/quality/v2/qbs/export?appCode=${ this.projectCode }&projectName=${encodeURIComponent(`${this.projectConfig?.projectName }`)}` ) }
      >
  ...
  private cancelMBSOpt() {
    ...
    mbsStateCollection.conditions = null;
    this.mbsBindStatus = 0;
  ...
  private renderMBSNav() {
    ...
    const defaultNav = [
      // <Button
      //   type={'primary'}
      //   onClick={ e => this.openImportModal('code_mbs') }
      // >
      //   { locale?.Common.Action.import }
      // </Button>,
      // <Button
      //   type={'primary'}
      //   onClick={ e=> { window.open(`/api/api/code_mbs/data_export?appCode=${ this.projectCode }&projectName=${ this.projectConfig?.projectName }`)} }
      // >
      //   { locale?.Common.Action.export }
      // </Button>,
      <Button
        ...
        type={'primary'}
      >
        { locale?.Engineering.relevanceSet }
      </Button>
    ];
    const tips = (
    ...
    const operationNav = [
      ...
      <Select
        defaultValue={0}
        ref={ 'RelevanceBtn' }
        onChange={this.getRelevanceStatus}
      >
        <Select.Option value={0}>
          <Tooltip title={locale?.Progress.All}>
            {locale?.Progress.All}
          </Tooltip>
        </Select.Option>
        <Select.Option value={1}>
          <Tooltip title={locale?.Engineering.HasRelevance}>
            {locale?.Engineering.HasRelevance}
          </Tooltip>
        </Select.Option>
        <Select.Option value={2}>
          <Tooltip title={locale?.Engineering.NoRelevance}>
            {locale?.Engineering.NoRelevance}
          </Tooltip>
        </Select.Option>
      </Select>,
      <Button
        type={ 'primary' }
        loading={ mbsStateCollection.mountLoading }
        disabled={ !(this.mainTable.selectRow?.level===QBSLevel.ITEM_PROJECT)}
        onClick={ ()=> this.openMountNodesModal() }
      >
      ...
      <Button
        ref={ 'RelevanceBtn' }
        type={ 'primary' }
        disabled={ !(this.mainTable.selectRow?.level===QBSLevel.INSPECTION_LOT)}
  ...
  private renderMBSTreeNav () {
    ...
    const defaultNav = [
      ...
      <Button
        type={'primary'}
        onClick={ e=> { window.open(`/api/api/code_mbs/data_export?appCode=${ this.projectCode }&projectName=${ encodeURIComponent(`${this.projectConfig?.projectName }`)}`)} }
      >
        { locale?.Common.Action.export }
      </Button>
    ];
  ...
  private renderMBSMainNav() {
    ...
    return [
      <Input.Search
        ...
        placeholder={ locale?.Engineering.searchPlaceHolder }
        loading={ mbsStateCollection.loading }
        value={ mbsStateCollection.conditions }
        onChange={ e => this.mbsStateCollection.conditions = e.target.value }
        enterButton
        onSearch={ e => this.getMBSTree()}
      />
    ]
  }
  ...
  private qbsClickRow({ row }: { row: Type.QualityTable } ) {
    //非关联设置状态双击（非两下）/关联设置状态
    if(row.id===this.mainTable.selectRow?.id && !this.mbsStateCollection.operation) {
      this.qbsDBClick();
      // this.getMBSTree();
    }else if(!this.isLoadQbsTree){  //处理懒加载会执行点击事件问题
      this.qbsChange({ row });
    }
  }
  ...
  * */
  ...
  mounted() {
    ...
    this.getQBSTree()?.then( () => {
      if(!this.mainTable.dataSource.length) {
        this.getMBSTree()
      }
      return this.autoSetFirstInit();
    } );
