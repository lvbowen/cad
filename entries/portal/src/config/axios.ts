...
axios.interceptors.request.use(
  config => {
    ...
    if (
      ...
      axios
        .get(`${env.apiHost}/login/Authentication/get_refresh_token`, { params })
        .then((res: any) => {
          // console.log(res);
          if (res) {
...
axios.interceptors.response.use(
  response => {
    ...
    if (response.headers['content-disposition']) {
      ...
      // @ts-ignore
      sessionStorage.setItem(response.config.url.split('/')[response.config.url.split('/').length-1],fileName)
    }
      ...
      switch (error.response.status) {
        case 401:
          if (
            ...
            if(window.location.href.indexOf('ModelAnnotation')>-1) return
            if (corpId) {
              url = `${url}?corpId=${corpId}`;
            }
            if (sessionStorage.getItem('isExternal')) {
              window.location.href = `${window.config.portalHost}/loginExternal`
            }else if(window.location.href.indexOf('RelayRoute')>-1){
              return
            }else if(window.location.href.indexOf('administratorLogin')>-1) {
              return
            }else{
              window.location.href = `${window.config.portalHost}${url}`;
            }
          }
