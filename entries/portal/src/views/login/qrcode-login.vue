...
export default class LoginQrcode extends Vue {
 ...
 @PropSync('corpId') syncCorpId!: string;
  ...
  generateUrls() {
    ...
    // 组装地址
    this.redirectUrl = `${oauthHost}/login/dingtalk?redirect_uri=${encodeURIComponent(`${oauthHost}/oauth/authorize?client_id=${client_id}&response_type=code&scope=${scope}&redirect_uri=${redirectHost}/oauth`)}&login_fail_redirect_uri=${encodeURIComponent(failUrl)}&corpId=${this.syncCorpId}`;
    // 组装二维码请求地址参数
