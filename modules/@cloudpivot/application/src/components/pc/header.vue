<template>
  <ul class="application-menu">
    <li @click="setSliderPos">
      <router-link to="/workflow-center">
        <span :title="$t('languages.common.workflowCenter')">{{ $t('languages.common.workflowCenter') }}</span>
      </router-link>
    </li>
    <li
    ...
    <li class="more" v-if="isShowMoreApps">
      <a-dropdown :trigger="trigger" @visibleChange="visibleChange" v-model="visible">
...
import { Dropdown, Icon, Input } from '@h3/antd-vue';
...
export default class CommonHeader extends Vue {
  ...
  getDownloadUrl(refId:string){
    let url = `${this.apiHost}/api/aliyun/download?refId=${encodeURIComponent(refId)}`;
    if(this.token){
