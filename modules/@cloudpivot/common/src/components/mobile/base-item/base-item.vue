<template>
  <div class="card-item" @click="$emit('click')">
    ...
    <section class="item-content">
      <div class="head-wrap">
        <img
          v-if="isTrustStart && !isTrustor"
          class="delegation-icon"
          src="./entrusted.png"
        >
        <img
          v-else-if="isTrustStart && isTrustor"
          class="delegation-icon"
          src="./entrust.png"
        >
...
export default class CardItem extends Vue {
  ...
  @Prop({ default: false }) isTrustStart?: boolean;

  @Prop({ default: false }) isTrustor?: boolean;

  // 是否为头像占位，当头像空白时
  @Prop() avatarPlacehold?: boolean;

  @Prop({ default: true }) showCreater?: boolean; // 是否展示发起人
