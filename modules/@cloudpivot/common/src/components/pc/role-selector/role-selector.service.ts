...
class AbstractRoleService {
  ...
  /**
   * 请求接口获取角色组列表
   * @param params: expandAll是否展开所有角色组, roleType, // 是否过滤主管角色
   */
  async getGroupList(params?: boolean, filterDD?:boolean, ) {
    let groups: Array<RoleSelectorTypings.RoleGroupItem> = [];
    const res: any = await organizationApi.getRoleGroupList2(params);
    if (res.errcode === 0 && Array.isArray(res.data)) {
      let data = res.data;
      // 关联组织新增角色时不能选择钉钉角色
      if (filterDD) {
        data = data.filter((item:any) => item.roleType !== 'DINGTALK')
      }
      groups = this.generateGroups(data);
    } else {
   ...
   * @param roleType 过滤角色类型
   */
  async searchRoles(params: any) {
    // searchRolesByName
    const { roleName } = params;
    let roles: Array<RoleSelectorTypings.RoleItem> = [];
    const res: any = await organizationApi.searchRolesByName(params);
    if (res.errcode === 0 && Array.isArray(res.data)) {
  ...
  async getRoleInfoByID(roleId: string) {
    const res: any = await organizationApi.getRoleGroupById(roleId);
    let roleInfo: RoleSelectorTypings.RoleItem | null = null;
    if (res.errcode === 0 && res.data) {
      const { groupId,code,name } = res.data;
      roleInfo = {
        groupName: '',
        groupId: groupId,
        code: code,
