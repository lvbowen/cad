  ...
  >
    ...
    <div class="role-select__search">
      ...
      <input
        ...
        @input="searchRole"
      />
      <i v-if="searchTxt" class="aufontAll h-icon-all-close-circle" @click="clearSearch"></i>
    </div>
    <!-- 三栏 -->
...
export default class RoleSelector extends Vue {
   ...
   */
  async getGroups() {
    this.roleGroups = await RoleService.getGroupList({ expandAll: this.expandAll, ...this.params, filterDefaultRoleGroup: this.filterDefaultRoleGroup }, this.filterDD,);
  ...
  async searchRole() {
    ...
    this.searchRoles = [];
    const searchRoles: any = await RoleService.searchRoles({ roleName: this.searchTxt, ...this.params, filterDefaultRoleGroup: true, filterDingRoleGroup: true });
    this.$nextTick(() => {
  ...
  doSelectRole(role: RoleSelectorTypings.RoleItem, checked: boolean) {
    ...
    if (checked) {
      if (this.multiple) {
        if (!this.selected.roles.some((item: RoleSelectorTypings.RoleItem) => item[filterKey as string] === role[filterKey as string])) {
      ...
      if (this.multiple) {
        roles = this.selected.roles.filter((item: RoleSelectorTypings.RoleItem) => item[filterKey as string] !== role[filterKey as string]);
      } else {
        roles = [];
      }
  ...
  isSelectedRole(role: RoleSelectorTypings.RoleItem) {
    ...
    return this.selected.roles.some((item: RoleSelectorTypings.RoleItem) => item[filterKey as string] === role[filterKey as string]);
  }
