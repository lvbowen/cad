<template>
  ...
  >
    <div class="workflow-group-content">
      ...
      <h4 :title="$i18n.locale === 'zh' ? data.name : data.name_i18n[$i18n.locale]">{{ $i18n.locale === 'zh' ? data.name : data.name_i18n[$i18n.locale] }}</h4>
...
export default class WorkflowGroup extends Vue {
  ...
  @Prop({default: 1}) penterIndex!:number;

  @Prop({default: 1}) groupIndex!:number;
  ...
  created() {
    this.curIconBg = this.workflowIconRandom(this.iconBgArray, this.groupIndex, this.penterIndex);
    this.curIcon = this.workflowIconRandom(this.iconArray, this.groupIndex, this.penterIndex);
  }
  ...
  * 随机生成应用分组图标
  */
  workflowIconRandom(arr:Array<string>, index:number, index2:number) {
    const randomIndex = index%arr.length;
    return index2%2 === 0 ? arr[randomIndex] : arr.reverse()[randomIndex];
  }
}
</script>
