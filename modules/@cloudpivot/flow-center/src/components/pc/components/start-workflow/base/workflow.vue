<template>
  ...
  >
    ...
    <div
      ...
      :title="data.name_i18n[$i18n.locale]"
    ></div>
...
export default class Workflow extends Vue {
  ...
  getWorkflowList(){
    return this.$store.dispatch('StartWorkflowStore/getWorkflowList', this.appCode);
  }
  ...
  async setFavorite(evt: Event) {
    ...
    if (this.favoriteClass) {
      ...
      if (!res.errcode) {
        ...
        this.$message.warning(this.$t('cloudpivot.flowCenter.pc.sevenFlowsIsMost') as string);
      }
    }
  }
  ...
  */
  onClick(code: string) {
    ...
    if (this.isDingTalk) {
      this.$router.push(route).catch((err: any) => {err});
    } else {
