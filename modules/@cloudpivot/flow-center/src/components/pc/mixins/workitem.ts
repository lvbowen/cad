  ...
  */
  appendLoadAll() {
    ...
    this.$nextTick(() => {
      ...
      (document.querySelector('.common-table__tbody-wrap') as any).appendChild(loadAllBox);
    });
  }
  ...
  */
  removeLoadAll() {
    this.$nextTick(() => {
      ...
      if (loadAllBox) {
        (document.querySelector('.common-table__tbody-wrap') as any).removeChild(loadAllBox);
      }
    });
  }
  ...
  */
  ...
  tableScrollerHandler(){
    this.$nextTick(() => {
      ...
      const table:any = document.querySelector('div.common-table') as HTMLElement;
      table.onmouseenter = function(){
        table.className = 'common-table active';
      }
  ...
  setTableMaxHeight(){
    ...
    this.$nextTick(() => {
      ...
      if (isTrident) {
        ...
        this.scrollY = doc.querySelector('.table').clientHeight;
        (doc.querySelector('.ant-table-body') as any).style.maxHeight = doc.querySelector('.table').clientHeight + 'px';
        return ;
      }
    })
  }
  ...
  */
  ...
  nameTranslator(key:string, type:string){
    ...
    switch(key) {
      ...
      case 'activityName':
        return type === 'en' ? "approvalNode" : "审批节点";
      case 'participant':
        return type === 'en' ? "Resolver" : "处理人";
      case 'instanceState':
        return type === 'en' ? "Workflow Status" : "流程状态";
      case 'workflowTplName':
        return type === 'en' ? "Workflow mode" : "流程模板";
      case 'sequenceNo':
        return type === 'en' ? "Squence No" : "单据号";
      default:
        return '';
    }
  }
