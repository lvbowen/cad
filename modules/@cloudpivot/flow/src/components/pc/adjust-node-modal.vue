<template>
  <div class="adjust-node-modal">
    <div class="row">
      ...
      <a-radio-group class="ct" @change="change" v-model="value">
        <a-radio v-if="workflowState !== 'COMPLETED'" :value="1">{{ i18nData.finishWorkflow }}</a-radio>
        <template v-if="workflowState !== 'COMPLETED' && showOperat">
  ...
  handleChange() {
    ...
    this.nodes.find((node: any) => {
      if (node.activityCode === this.selectValue) {
        node.participantors && node.participantors.forEach((p: any) => (p.type = 3));
