...
import { H3Modal, H3SearchBar, H3Signature } from "h3-mobile-vue";
...
export default class SmartLocation extends Vue {
  ...
  mounted() {
    ...
    window.addEventListener('popstate',this.cancelWithOutGoBack);
  }

  beforeDestroy(){
    window.removeEventListener('popstate',this.cancelWithOutGoBack);
  }
  ...
  onChange(val: any) {
    if (val) {
      this.address = `${val.provinceName || ""}${val.cityName || ""}${
        val.districtName || ""
      }${val.address || ""}`;
      //  val.address || '';
    }
