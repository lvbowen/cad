  ...
  >
    ...
    <div class="verify-item" v-show="!isQuantity" style="margin-bottom: 20px">
      ...
      <div class="content" v-show="isDynamicDay">
        ...
        />
        <div class="content" v-show="isRangeRule" style="align-items: center">
          <a-input-number v-model="numInput1" style="width:100%" placeholder="请输入数字" />
          <div style="margin: 0 6px;color:rgba(0,0,0,0.65);font-size: 14px;">~</div>
          <a-input-number v-model="numInput2" style="width:100%" placeholder="请输入数字" />
        </div>
      </div>
      <div class="content" v-show="!isDynamicDay">
        <!--非介于-->
...
export default class VerifyFormulaDate extends Vue {
  ...
  created() {
        ...
        while (loop.length) {
          ...
          for (let _panel of _tabs.panels) {
            for (let _ctrlKey of Object.keys(_panel.controls)) {
              ...
              if (
                _ctrl.type === FormControlType.Date &&
    ...
    this.selectListDataItems = this.dateControlOptions.filter(
