        ...
        while(loop.length) {
          ...
          for(let _panel of _tabs.panels) {
            for(let _ctrlKey of Object.keys(_panel.controls)) {
              ...
              if (_ctrl.type === FormControlType.Number && _ctrlKey !== this.dataItem.code ) {
                  this.selectListDataItems.push(Object.assign({},_ctrl.options,{code:_ctrlKey}))
                } else if (!this.currControlOptions && _ctrlKey === this.dataItem.code) {
                  this.currControlOptions = _ctrl.options
                } else if (this.isChild && _ctrl.type === FormControlType.Sheet && this.dataItem.parentCode === _ctrlKey) {
                  _ctrl.columns.forEach((subItem) => {
                    if (subItem.type === FormControlType.Number) {
                      if (subItem.key !==  this.dataItem.code) {
                        this.selectListDataItems.push(Object.assign({},subItem.options,{code:_ctrlKey+'.'+subItem.key}))
                      } else if (!this.currControlOptions && subItem.key === this.dataItem.code) {
                        this.currControlOptions = subItem.options
                      }
                    }
