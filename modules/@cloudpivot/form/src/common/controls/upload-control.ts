...
export abstract class UploadControl<T extends typings.UploadOptions> extends BaseControl<T> {
  ...
  checkFileType(name: string, showError?: (msg: string) => void) {
    ...
    if (this.control.type === typings.FormControlType.Image) {
      ...
      if (!is && showError) {
        msg = this.$t(
          "cloudpivot.form.renderer.tip.imageSuport", {
      ...
      if (/[,,]/.test(name)) {
        if (showError) {
          msg = this.$t(
            "cloudpivot.form.renderer.tip.imageNameCHeck"
          ).toString();
          showError(msg);
    ...
    if (ignore_file_types.indexOf(ext) > -1) {
      if (showError) {
        msg = this.$t(
          "cloudpivot.form.renderer.tip.fileUnsuport", {
          label: ext
        }
        ).toString();
        showError(msg);
    ...
    if (/[,,]/.test(name)) {
      if (showError) {
        msg = this.$t(
          "cloudpivot.form.renderer.tip.fileNameCheck"
        ).toString();
        showError(msg);
      }
      return false;
    ...
    if (types.indexOf(ext) < 0) {
      if (showError) {
        msg = this.$t(
          "cloudpivot.form.renderer.tip.fileSuport", {
          label: types.join("、")
        }
        ).toString();
        showError(msg);
