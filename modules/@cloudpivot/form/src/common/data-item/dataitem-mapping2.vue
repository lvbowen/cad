  ...
  onTargetItemChange(selectedCode: string, item: DataitemMappingValueItem) {
    ...
    if (typeof item.target !== 'string') {
      if (item.target.code !== selectedCode) {
        ...
        const selectedData = this.targetOptions.find(res => res.value === selectedCode);
        // item.target.code = selectedData.value
        if (selectedData) {
          item.target = selectedData.item;
        }
        
        // if (selectedData && typeof selectedData !== 'string') {
        //   item.target = selectedData;
        // }
      }
    }
    this.onChange();
