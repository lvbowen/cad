...
export abstract class FromAddressValueService {
  ...
  static async setAddressVal(adcode:string,format?:string) {
    const value:any[] = [];
      if (!format) {
        format = 'pp-cc-aa';
      }
    ...
    // const mod = await import(/* webpackChunkName: "pca" */'../components/shared/pca-selector/typings/pca');
    const mod = await import(/* webpackChunkName: "pca" */'@cloudpivot/form/src/common/components/pca-selector/typings/pca');
    ''
    mod.default.forEach((p:any) => {
