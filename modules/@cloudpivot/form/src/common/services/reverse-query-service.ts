...
import { DataItemType, RelevanceFormSelectMode } from "@cloudpivot/form/schema";
...
export abstract class ReverseQueryService {
                ...
                if (row[col.key] !== undefined) {
                  if (match) {
                    if (key === "sequenceStatus") {
                      ...
                      }else if(col.type === typings.FormControlType.Image || col.type === typings.FormControlType.Attachment){
                        /**
   ...
   * @param value 
   */
  static convertFileData(value: any) {
    let result;
