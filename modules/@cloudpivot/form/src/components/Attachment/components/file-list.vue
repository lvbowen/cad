    ...
    >
      ...
      <div class="file-item-text-warp">
        <div class="file-item-text">
          <a-tooltip>
            <template slot="title">{{ item.name }} ({{ initSize(item) }})</template>
            <p
              class="file-item-name"
              :title="item.name"
            >{{ initFieName(item.name) }} ({{ initSize(item) }})</p>
          </a-tooltip>

          <p class="file-item-name-right">
            <span>{{ getName(item) }}</span>
            <span class="icon-wrap">
              <span style="padding-right:10px">{{ getTime(item) }}</span>
              <a-icon type="download" @click="downFile(item, key,'download')" />
...
export default class FileList extends Vue {
  ...
  @Emit()
  rmFile(file, key) {
    let res = { ...file, status: UploadStatus.Delete };
    // // 更新文件上传状态list
  ...
  initSize(item) {
    if (item.size !== undefined) {
      let size = item.size ? item.size : item.response.data.fileSize;
      let k = 1024; // 比特 字节大小
      let m = 1048576; // 兆字节大小
      let num = k; // 默认使用的几算大小
      let str = "KB";
      ...
      let size = item.fileSize ? item.fileSize : item.response.data.fileSize;
      let k = 1024; // 比特 字节大小
      let m = 1048576; // 兆字节大小
      let num = k; // 默认使用的几算大小
      let str = "KB";
