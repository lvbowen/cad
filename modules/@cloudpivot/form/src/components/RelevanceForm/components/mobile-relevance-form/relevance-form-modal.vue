<template>
  <div class="relevance-form" v-show="visible">
    <popup
      @close="close"
      v-model="visible"
      position="bottom"
...
import {Popup} from '../../../../../../../../entries/mobile/node_modules/vant';
...
export default class RelevanceFormModal extends RelevanceFormControl {
    ...
    if (this.visible) {
      this.getListQueryConditions(true).then((res: any) => {
        if (res) {
          ...
          this.inputName = this.queryConditions[0].name
        } else {
  ...
  inputValue(val) {
    this.inputValueArr = this.queryConditions.map((item => {
      return {
    ...
    this.inputValueArr[0].propertyValue = val
    this.onFilter(this.inputValueArr)
  }
