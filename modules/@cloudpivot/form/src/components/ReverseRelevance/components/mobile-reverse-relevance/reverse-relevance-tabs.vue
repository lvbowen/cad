...
import { RelevanceFormControl, RelevanceFormSearchParams, RelevanceFormSearchResult } from "@cloudpivot/form/src/common/controls/relevance-form-control";

// import  from "../relevance-form/search-panel.vue";

import SearchPanel from "@cloudpivot/form/src/components/RelevanceForm/components/mobile-relevance-form/search-panel.vue"
...
.reverse-relevance-add{
    ...
    z-index: 99;
    right: 0.53rem;
    bottom: 2.3rem; //transform属性影响固定定位并导致定位不准确，暂时未想到好的解决方案，只能通过把bottom定位高一点来解决（造成的问题为在查看状态与编辑状态按钮的高度会不一致）
    img{
      height: 1.33rem;
      width: 1.33rem;
    }
