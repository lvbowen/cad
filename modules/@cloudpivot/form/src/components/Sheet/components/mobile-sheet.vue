...
<template>
  <div
    ...
    <div class="h3-form-sheet-head sheet-row" :class="{'h3-form-sheet-head_sticky' : !isHorizontal }">
      <h3-field :required="control.required" :label="label" :showIcon="false"></h3-field>
    ...
    <h3-modal
      ...
      <div class="h3-form-sheet-search-list">
        <span
          ...
          :class="{ selected : currentIndex === key }">{{ key }}</span>
      </div>
      <template v-if="control">
        <relevance-form-modal
          ...
          :columns="columns"
          :query="query"
          @back="close"></relevance-form-modal>
      </template>
    </h3-modal>
            ...
            <thead>
              <tr>
                ...
                <th
                  ...
                  :style="getColumnStyle(col)">{{ getColumnLabel(col) }}</th>
              </tr>
            </thead>
                ...
                <td
                  ...
                  <div>
                    <base-control-adapter
                      ...
                      :control="col"
                      :isSheetTable="true"
                      :class="{ error : col.controller.hasError }"></base-control-adapter>
                  </div>
                </td>
              </tr>
      ...
      <div class="h3-form-sheet-action sheet-row" v-show="isHorizontal">
        <div class="h3-form-sheet-action-tab" ref="selectedTab">
          <span
            ...
            v-for="key in sheetTab"
            :key="key"
            @click="changeTab(key - 1)">{{ key }}</span>
        </div>
