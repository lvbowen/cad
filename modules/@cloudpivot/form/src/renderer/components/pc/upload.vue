<template>
  <div class="upload-control-component" :class="{ 'upload-image': isImage, 'upload-files': !isImage }">
    <!-- batch-download-o -->
    ...
    <template v-if="showBatchDownload && !!isReadonly && fileList.length > 1">
      ...
      <a-button @click="downloadAll" class="download" icon="batch-download-o">{{ $t('cloudpivot.form.renderer.downloadAll') }}</a-button>
    </template>
    ...
    <a-upload
      ...
      <template v-if="canUpload && !isReadonly && !controlOpts.onlyShoot">
        ...
        <template v-else>
          ...
          <div class="all-download-wrap" v-if="showBatchDownload && fileList.length > 1 && canDounload" @click.stop="downloadAll">
            <span @click.stop="downloadAll" class="all-download icon aufontAll h-icon-all-batch-download-o"></span>
...
export default class PcUpload extends FileUploadControl {
  ...
  handleValueChange(value: any, changeInfo: any): void {
    ...
    // #43067 上传一个文件点删除按钮后（value.length === 0）时需要调用this.toFilelist
    if (!value) {
    ...
    else if (value.length && (!this.uploadFileList || this.uploadFileList.length === 0)) {
      this.toFilelist(value);
  ...
  beforeUpload(file: any) {
    ...
    file.uploadName = window.sessionStorage.getItem('uploadName') || userInfo.name;
    return true;
  }
  ...
  onChange(info: { file: any; fileList: any[]; event: any }): void {
    ...
    this.uploadFileList = info.fileList.filter(f => f.status && f.status !== "removed")
    //保存文件状态
    let { file } = info;
    // 保存文件上传进度
    this.eventList = info.fileList;
    const files = info.fileList
      .filter(f => f.status && f.status !== "removed")
      .map(f => {
