...
export default class SmartOrg extends Vue {
  ...
  get hasValue() {
    let excelType = this.selectedData.some((item: any) => item.source && item.source.excelType);
    return !excelType && this.selectedData && this.selectedData.length > 0;
  }
  ...
  selectAll(val: any, oldVal: any) {
    ...
    // 解决 添加子管理员时，部门列表为空时，也全选上的问题（负责人：周龙秋）
    if (val.length) {
      isSelectAll = val.every((item: any) => item.isSelected)
    }
