...
export abstract class FormControlOptionsService {
   ...
   */
  static buildFor(
    controlType: typings.FormControlType,
    sourceOpts?: typings.FormControlOptions
  ) {
    let options: typings.FormControlOptions | null = null;
    const op = getComponentOptionsByControlType(controlType); // 获取组件的options
    if (op) {
      options = op;
    } else {
      switch (controlType) {
        ...
        case typings.FormControlType.RelevanceFormEx:
        ...
        case typings.FormControlType.Tabs:
          options = new typings.TabsOptions();
          break;
