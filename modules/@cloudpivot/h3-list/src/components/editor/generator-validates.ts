...
class ListMobileTableGenerator extends ListHtmlGenerator {
  ...
  async generateTemplate() {
    ...
    if ( options.queryPresentation && options.queryPresentation.htmlJson ) {
      const htmlJson = typeof(options.queryPresentation.htmlJson)==='string'? JSON.parse(options.queryPresentation.htmlJson): options.queryPresentation.htmlJson;
      let templateJson;
    ...
    if ( !template || !template.trim() ) {
      const converter = this.converter || (this.converter = ((await import('@cloudpivot/list/src/components/listCustom/template/mobile/handler')) as any).default);
