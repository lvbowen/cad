...
export class ListHtmlGenerator {
  ...
  private validatePresentation() {
    ...
    if (this.options.queryPresentation) {
      ...
      this._presentation.htmlJson = htmlJson ? ListElementGeneratorService.parseString(htmlJson) : '';
      this._presentation.actionsJson = actionsJson ? ListElementGeneratorService.parseString(actionsJson) : '';
      this._presentation.columnsJson = columnsJson ? ListElementGeneratorService.parseString(columnsJson) : '';
    }
  }
  ...
  private generateStyle() {
    const tagName = 'style';
    const attrs = [{ name:'id', value:'customStyle' }];
    let inner = '\n';
    if (this._presentation.htmlJson && typeof this._presentation.htmlJson !== 'string') {
  ...
  private generateActions() {
    ...
    if (this._actions.length) {
      ...
      this._actions.forEach((action: ActionElement) => {
        ...
        const str = `${before}${this.generateAction(action)}${after}`;
        inner += str;
      })
    }
    this._actionSection = `\n<!-- 自定义按钮:  -->\n<!--
  ...
  private generateCustomTemplate() {
    ...
    const comment = `<!-- ${$t('customTemplateTopComment')} -->\n`;
    let inner   = '';
  ...
  private generateScriptVersionLogs(version) {
    const handler       = this.listEventHooksHandler || (this.listEventHooksHandler=require('@cloudpivot/list/src/components/listCustom/eventHooks/handler').default);
    const versionClass  = handler.loadSupportingVersionClass(version);
  ...
  private combineContent() {
    const isPC = this.options.clientType === 'PC';
    this._htmlContent =
    '<!--' +
    '\n\t******************************** 自定义视图: ********************************' +
