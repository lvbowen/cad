...
export default class ListEventHooksHandler {
  ...
  constructor(options:constructorOptions) {
    const { vm, scription } = options;
    const { hooksOption=[] } = options;
  ...
  exec(event, action?:Actions, result?:any) {
    const vm          = this.vm;
    const scription   = this.scription;
    const eventFunc   = scription.hooks[event];
    const eventOption = this.hooksOption.find(h=>h.name===event) as any;
    ...
    const eventFuncString = String(eventFunc);
  ...
  public static parseScriptString(scriptString:string) {
    ...
    const version = scription.options.version;
    if ( version!=='1.0' ) {
      const versionClass = this.loadSupportingVersionClass(version);
  ...
  public static loadSupportingVersionClass(clientVersion) {
    const handlerVersion = (this.versionSupportingList.find(v=>v.version===clientVersion) || {as:'undefined'}).as;
  ...
  public static loadSupportingVersionHandler(options:subClassLoaderOptions) {
    ...
    const { scriptString } = options;
    const scription      = this.parseScriptString(scriptString);
    const clientVersion  = scription.options.version;
    const versionClass   = this.loadSupportingVersionClass(clientVersion);
    const versionHandler = new versionClass({...options, scription});
