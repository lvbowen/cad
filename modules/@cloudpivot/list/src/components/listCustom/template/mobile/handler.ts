...
export default class MobileListTemplate {
  ...
  public parseHtml(html:string) {
    ...
    const virtualDom = new (window as any).DOMParser().parseFromString(html,'text/html');
    const templateWrapper = virtualDom.body.querySelector(MobileListTemplate.templateSelector);
