  ...
  <div class="search">
    ...
    <div class="search-content" v-show="currentWay">
      <div class="search-content-status" v-show="currentWay === 'status'">
        ...
        <div class="buttons">
          <h3-button type="primary" @click="submitStatus">{{
            $t("cloudpivot.list.mobile.OK")
          }}</h3-button>
        </div>
      </div>
      <list-design-search
...
export default class ListFormSearch extends Vue {
  ...
  logStatusNames() {
    this.currentStatusName = this.formStatus
      .map((item: any) => item.checked && item.name)
      .filter(Boolean);
  }
  ...
  switchWay(item: any) {
    if (item.type === "filter" && !this.queryConditions.length) {
      ...
      console.log(this.relevanceDataList);
    }
  }
  ...
  submitStatus() {
    ...
    formStatus.propertyValue = targetArr
  ...
  setFilterStatus() {
    this.queryConditions.forEach((con: any) => {
      if (con.propertyCode === "sequenceStatus") {
        (this.$refs.filters as any).setFilterValue(
          "sequenceStatus",
  ...
  reSetStatus(filters: any) {
    const sequenceStatus: any = filters.find(
      (filter: any) => filter.propertyCode === "sequenceStatus"
    );
