<template>
  <div class="data-import-status">
    ...
    <div class="data-import-status--detail" v-else-if="showFailMessage">
      ...
      <p class="data-import-status--detail__message" v-else-if="fileTypeError">
        {{ $t('cloudpivot.form.renderer.tip.ImportTips3') }}
      </p>
...
export default class DataImportStatus extends Vue {
  ...
  get showFailMessage() {
    return this.status === listParams.ImportResult.DataNumExceed
     ...
     || this.status === listParams.ImportResult.FileTypeError
     || this.isUnspecified
     || this.isTemplateEmpty;
  }

  get fileTypeError() {
    return this.status === listParams.ImportResult.FileTypeError;
  }
