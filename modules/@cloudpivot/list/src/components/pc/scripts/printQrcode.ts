...
export default {
  ...
  */
  async createPrintQrCodeData(vm:any, data: any) {
    ...
    //前端里xmjc设置为空时，getQueryList才有返回结果
    for (const item of vm.filterData){
      if (item.propertyCode === 'xmjc_1' && item.propertyValue !== ''){
        item.propertyValue = '';
      }
    }
    ...
    if (res.errcode === 0 && res.data) {
      ...
      // 调用接口批量生成短码
      const projectCode = schemaCode.split("_")[0];
      ...
      if (genShortCodesRes.errcode === 0 && genShortCodesRes.data) {
        ...
        dataArr.forEach((item:any, index:number) => {
          const text:string = `${window.location.protocol}//${window.location.host}/${projectCode}/mobile/el.html?c=${otherData[index].pairCode}&corpId=main&agentId=${config.agentId}`;
          const url:string = `${env.apiHost}/api/qrcode/generate_qrcode?text=${encodeURIComponent(text)}`;
          item.url = url;
        })
      }
    }
  ...
  printQrCode(vm:any, result: any) {
    ...
    if (isTrident) {
      window.localStorage.setItem('printData', JSON.stringify(result));
      window.open('/print');
      return ; 
    }
    vm.opts = result;
