...
export default {
  ...
  handleColumResizeEnd(colum: AllTypes.ColumnOption, recordKey:string, columnsOptsKey:string){
    const { width, column, parentColumn } = colum;
    const colKey: string = parentColumn ? `${parentColumn.id}.${column.id}`: column.id;
    ...
    if (columnOpts) {
      ...
      const colItem: AllTypes.DisplayedColumn = cols.find((col: AllTypes.DisplayedColumn) => {
        if (parentColumn && parentColumn.id === col.id && Array.isArray(col.childColumns)) {
          childItem = col.childColumns.find((c:any) => `${parentColumn.id}.${c.id}` === colKey);
          return false;
        } else {
          return col.id === colKey;
        }
      }) as AllTypes.DisplayedColumn;
      if (colItem) {
        colItem.width = width;
      } else if (childItem) {
