...
class Utils {
    ...
    public static cnReg = /[\u2E80-\u2EFF\u2F00-\u2FDF\u3000-\u303F\u31C0-\u31EF\u3200-\u32FF\u3300-\u33FF\u3400-\u4DBF\u4DC0-\u4DFF\u4E00-\u9FBF\uF900-\uFAFF\uFE30-\uFE4F\uFF00-\uFFEF]+/g;

    public static deepFind<T>( arrData: Array<T>, condition: ( item, i, level ) => boolean, children: string ) {
        const main: Array<T> = []
        try {
    ...
    public static GetRequest(): UrlParam {
    ...
    public static getFullHeight( Element: CSSStyleDeclaration, unit: string, way: 'add' | 'minus' ): number {
    ...
    public static debounce: ( fn, ms: number ) => Function = ( fn, ms = 500 ) => {
        let timeoutId;
    ...
    public static debounceFn: ( fn, wait: number ) => Function = ( fn, wait = 500 ) => {
        let timeoutId;
        return ( v ) => {
            if ( timeoutId ) {
                clearTimeout( timeoutId );
    ...
    public static isAmount: ( value: string ) => false | number = ( value ) => {
    ...
    public static generateXTableColumns: <T>( columns: Array<T> ) => Array<T> = <T>( columns ) => {
    ...
    public static adapterImpl<T>( dataSource: Array<T>, childrenKey: string ): void {
    ...
    public static treeStructureAdapter<T>( tree: Array<T>, treeField: string, collection?: Array<T>, autoSet?: boolean ): void | Array<T> {
