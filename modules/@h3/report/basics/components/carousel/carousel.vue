  ...
  >
    ...
    <div v-if="isEdit">
      ...
      <div style="margin-top: 5px">
        ...
        <a-button size="small" @click="nextPic" style="margin-left: 0.5vw"> {{ picShow }}
          <a-icon type="right"/>
        </a-button>
      </div>
    </div>
    <div class="ql-editor" v-else>
      <div class="ql-container">
        ...
        <a-carousel autoplay :dots="false" :style="isTitle?'height: 92%;width: 100%':'height: 100%;width: 100%'">
          <template v-for="(v,i) in picAll">
            <div v-html="v" :key="i"></div>
          </template>
        </a-carousel>
      </div>
    </div>
...
@Component({
  name: "h3-report-pic-play-wrap",
  components: {
    quillEditor,
    ACarousel: Carousel
  },
})
export default class PicPlayWrap extends Vue {
  @Prop() chart!: H3.Report.PicPlay;
  ...
  picAll: Array<string> = [];
  defaultPic: Array<string> = [];
  picShow: string = '';
  picNum: number = 0;//给图片组计数
  titleValue: string = '';
  ...
  setDefault() {
    ...
    this.defaultPic = array;
    let tempt: Array<any> = [];
    if (project.length === 0) {
      tempt = JSON.parse(this.defaultPic[0]).downloadUrl;
      this.picShow = JSON.parse(this.defaultPic[0]).name;
    } else {
      this.defaultPic.forEach((item) => {
        if (JSON.parse(item).name.indexOf(project) > -1) {
          this.picShow = JSON.parse(item).name;
          tempt = JSON.parse(item).downloadUrl;
        }
      })
    }
    this.picAll = tempt.map((item) => {
      return `<img src='${item}' style='height: 100%;width:100%;display:block'/>`
    })
  ...
  fileChange(files) {
    ...
    this.defaultPic = files;
    const tempt: Array<string> = JSON.parse(files[0]).downloadUrl;
    this.picShow = JSON.parse(files[0]).name;
    this.picAll = tempt.map((item) => {
      return `<img src='${item}' style='height: 100%;width:100%'/>`
    })
  }

  nextPic() {
    this.picNum = this.picNum < this.defaultPic.length - 1 ? this.picNum + 1 : 0;
    this.picAll = [];
    this.defaultPic.forEach((v, i) => {
      if (i == this.picNum) {
        this.picShow = JSON.parse(v).name;
        this.picAll.push(`<img src='${JSON.parse(v).downloadUrl}' style='height: 100%;width:100%'/>`);
        return;
      }
    })
  }
