...
function handleStripeData(options: H3.Chart.ChartOptions): H3.Chart.ChartData {
      ...
      item.forEach((cell: number, index: number) => {
        if (typeof cell === "number" && options.metricRange) {
          if (
            options.metricRange &&
            options.metricRange.max !== null &&
            cell > (options.metricRange as any).max
          ) {
            item[index] = options.metricRange.max;
          } else if (
            options.metricRange &&
            options.metricRange.min !== null &&
            cell < (options.metricRange as any).min
          ) {
  ...
  chartData.maxDimension = parseInt((maxDimensionColumns / chartData.groupLength).toString(), 10);
  chartData.maxDimension =
    chartData.maxDimension > maxDimension || isNaN(chartData.maxDimension)
      ? maxDimension
      : chartData.maxDimension;
